...
export class UnitTestManagementService
    implements ITestManagementService, Disposable {
    ...
    private readonly _onDidStatusChange: EventEmitter<
        WorkspaceTestStatus
    > = new EventEmitter<WorkspaceTestStatus>();
        ...
        if (testManager) {
            if (!this.testManagers.has(testManager)) {
                ...
                const handler = testManager.onDidStatusChange(e =>
                    this._onDidStatusChange.fire(e)
                );
    ...
    public async configurationChangeHandler(
        ...
        const workspaceFolderUri = this.workspaceService.workspaceFolders.find(
            w => eventArgs.affectsConfiguration("python.testing", w.uri)
        );
    ...
    private async registerSymbolProvider(
        symbolProvider: DocumentSymbolProvider
    ): Promise<void> {
        const testCollectionStorage = this.serviceContainer.get<
            ITestCollectionStorageService
        ...
        const handler = this._onDidStatusChange.event(e => {
            if (
                e.status !== TestStatus.Discovering &&
        ...
        this.disposableRegistry.push(
            activateCodeLenses(event, symbolProvider, testCollectionStorage)
        );
    }
    ...
    private registerCommands(): void {
        ...
        const disposables = [
            ...
            commandManager.registerCommand(
                ...
                (
                    _,
                    _cmdSource: CommandSource = CommandSource.commandPalette,
                    resource?: Uri
                ) => {
                    // Ignore the exceptions returned.
                    // This command will be invoked from other places of the extension.
            ...
            commandManager.registerCommand(
                constants.Commands.Tests_Discovering,
                noop
            )
        ];
