...
export class TestExplorerCommandHandler implements ITestExplorerCommandHandler {
    ...
    constructor(
        ...
        @inject(ITestDataItemResource)
        private readonly testResource: ITestDataItemResource
    ) {}
    public register(): void {
        this.disposables.push(
            this.cmdManager.registerCommand(
                Commands.runTestNode,
                this.onRunTestNode,
                this
            )
        );
        this.disposables.push(
            this.cmdManager.registerCommand(
                Commands.debugTestNode,
                this.onDebugTestNode,
                this
            )
        );
        this.disposables.push(
            this.cmdManager.registerCommand(
                Commands.openTestNodeInEditor,
                this.onOpenTestNodeInEditor,
                this
            )
        );
    }
    ...
    protected async runDebugTestNode(
        item: TestDataItem,
        runType: "run" | "debug"
    ): Promise<void> {
        ...
        const testUri = this.testResource.getResource(item);
        const cmd =
            runType === "run" ? Commands.Tests_Run : Commands.Tests_Debug;
        this.cmdManager.executeCommand(
            cmd,
            undefined,
