...
@injectable()
export class FailedTestHandler
    implements IExtensionActivationService, IDisposable {
    ...
    public onDidChangeTestData(args: { uri: Uri; data?: TestDataItem }): void {
        if (
            args.data &&
            (args.data.status === TestStatus.Error ||
                args.data.status === TestStatus.Fail) &&
