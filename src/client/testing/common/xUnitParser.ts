    ...
    return new Promise<any>((resolve, reject) => {
        fs.readFile(outputXmlFile, "utf8", (err, data) => {
            ...
            xml2js.parseString(
                ...
                (
                    error: Error,
                    parserResult: { testsuite: TestSuiteResult }
                ) => {
                    if (error) {
                        return reject(error);
                    }
                    try {
                        ...
                        tests.summary.errors = getSafeInt(
                            testSuiteResult.$.errors
                        );
                        tests.summary.failures = getSafeInt(
                            testSuiteResult.$.failures
                        );
                        ...
                        testSuiteResult.testcase.forEach(
                            (testcase: TestCaseResult) => {
                                ...
                                result.testFunction.time = parseFloat(
                                    testcase.$.time
                                );
