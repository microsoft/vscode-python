...
export class TestsDiscoveryService implements ITestDiscoveryService {
    constructor(
        ...
        private readonly execFactory: IPythonExecutionFactory,
        @inject(ITestDiscoveredTestParser)
        private readonly parser: ITestDiscoveredTestParser,
    ...
    public async discoverTests(options: TestDiscoveryOptions): Promise<Tests> {
        ...
        try {
            ...
            const discoveredTests = JSON.parse(
                output.stdout
            ) as DiscoveredTests[];
            ...
            if (output) {
                traceError(
                    "Failed to parse discovered Test",
                    new Error(output.stdout)
                );
            }
    ...
    public async exec(
        ...
        const execService = await this.execFactory.createActivatedEnvironment(
            creationOptions
        );
