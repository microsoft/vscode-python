...
export class TestFunctionCodeNavigator implements ITestCodeNavigator {
    ...
    @captureTelemetry(
        EventName.UNITTEST_NAVIGATE_TEST_FUNCTION,
        undefined,
        true
    )
    public async navigateTo(
        ...
        if (!range) {
            traceError(
                "Unable to navigate to test function",
                new Error("Test Function not found")
            );
            return;
        }
        if (focus) {
