...
export class TestSuiteCodeNavigator implements ITestCodeNavigator {
    ...
    constructor(
        @inject(ITestNavigatorHelper)
        private readonly helper: ITestNavigatorHelper,
    ...
    public async navigateTo(
        ...
        sendTelemetryEvent(EventName.UNITTEST_NAVIGATE_TEST_SUITE, undefined, {
            focus_code: focus
        });
        ...
        const [doc, editor] = await this.helper.openFile(
            Uri.file(item.parentTestFile.fullPath)
        );
        ...
        if (item.testSuite.line) {
            ...
            const predicate = (s: SymbolInformation) =>
                s.name === item.testSuite.name && s.kind === SymbolKind.Class;
        ...
        if (!range) {
            traceError(
                "Unable to navigate to test suite",
                new Error("Test Suite not found")
            );
            return;
        }
        if (focus) {
            ...
            await this.docManager.showTextDocument(doc, {
                preserveFocus: false,
                selection: range
            });
        } else {
