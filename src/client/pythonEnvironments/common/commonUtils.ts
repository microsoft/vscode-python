...
import { getPythonVersionFromConda } from '../base/locators/lowLevel/condaLocator';
import { getPythonVersionFromVenv } from '../base/locators/lowLevel/virtualEnvironmentIdentifier';
...
export async function getPythonVersionFromNearByFiles(interpreterPath:string): Promise<PythonVersion> {
    ...
    for await (const interpreter of findInterpretersInDir(root)) {
        try {
            const curVersion = parseExeVersion(interpreter);
