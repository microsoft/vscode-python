...
export class ThemeFinder implements IThemeFinder {
    ...
    public async findThemeRootJson(
        themeName: string
    ...
    public async findTmLanguage(language: string): Promise<string | undefined> {
        ...
        if (!this.themeCache.hasOwnProperty(language)) {
            try {
                this.languageCache[language] = await this.findMatchingLanguage(
                    language
                );
            } catch (exc) {
    ...
    private async findThemeData(
        themeName: string
    ): Promise<IThemeData | undefined> {
        // See if already found it or not
        if (!this.themeCache.hasOwnProperty(themeName)) {
            try {
                this.themeCache[themeName] = await this.findMatchingTheme(
                    themeName
                );
            } catch (exc) {
                this.logger.logError(exc);
    ...
    private async findMatchingLanguage(
        language: string
    ): Promise<string | undefined> {
        ...
        let results = await this.findMatchingLanguages(
            language,
            extensionsPath
        );
    ...
    private async findMatchingLanguages(
        language: string,
        rootPath: string
    ): Promise<string | undefined> {
        ...
        if (foundPackages.length > 0) {
            ...
            for (const f of foundPackages) {
                ...
                const data = await this.findMatchingLanguageFromJson(
                    fpath,
                    language
                );
                if (data) {
    ...
    private async findMatchingTheme(
        themeName: string
    ): Promise<IThemeData | undefined> {
        // Environment variable to mimic missing json problem
        if (process.env.VSC_PYTHON_MIMIC_REMOTE) {
        ...
        for (const e of extensions) {
            const result = await this.findMatchingThemeFromJson(
                path.join(e.extensionPath, "package.json"),
                themeName
            );
            if (result) {
                return result;
    ...
    private async findMatchingThemes(
        rootPath: string,
        themeName: string
    ): Promise<IThemeData | undefined> {
        // Search through all package.json files in the directory and below, looking
        ...
        if (foundPackages.length > 0) {
            ...
            for (const f of foundPackages) {
                const fpath = path.join(rootPath, f);
                const data = await this.findMatchingThemeFromJson(
                    fpath,
                    themeName
                );
                if (data) {
    ...
    private async findMatchingLanguageFromJson(
        packageJson: string,
        language: string
    ): Promise<string | undefined> {
    ...
    private async findMatchingThemeFromJson(
        packageJson: string,
        themeName: string
    ): Promise<IThemeData | undefined> {
        // Read the contents of the json file
