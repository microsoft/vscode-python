...
export class DataScience implements IDataScience {
    ...
    public async activate(): Promise<void> {
        ...
        this.changeHandler = this.configuration
            .getSettings()
            .onDidChange(this.onSettingsChanged.bind(this));
    ...
    public async runCell(
        ...
        if (codeWatcher) {
            return codeWatcher.runCell(
                new vscode.Range(startLine, startChar, endLine, endChar)
            );
        }
    }

    public async runAllCellsAbove(
        file: string,
        stopLine: number,
        stopCharacter: number
    ): Promise<void> {
        this.dataScienceSurveyBanner.showBanner().ignoreErrors();
    ...
    public async runCellAndAllBelow(
        file: string,
        startLine: number,
        ...
        if (file) {
            ...
            if (codeWatcher) {
                return codeWatcher.runCellAndAllBelow(
                    startLine,
                    startCharacter
                );
            }
    ...
    public async runFromLine(): Promise<void> {
        ...
        if (activeCodeWatcher && textEditor && textEditor.selection) {
            return activeCodeWatcher.runFromLine(
    ...
    private getCurrentCodeLens(): vscode.CodeLens | undefined {
        ...
        if (activeEditor && activeCodeWatcher) {
            ...
            return activeCodeWatcher
                .getCodeLenses()
                .find((c: vscode.CodeLens) => {
                    if (
                        c.range.end.line >=
    ...
    private onSettingsChanged = () => {
        ...
        let editorContext = new ContextKey(
        ...
        editorContext = new ContextKey(
            EditorContexts.OwnsSelection,
            this.commandManager
        );
    ...
    private getCodeWatcher(file: string): ICodeWatcher | undefined {
        const possibleDocuments = this.documentManager.textDocuments.filter(
            d => d.fileName === file
        );
        if (possibleDocuments && possibleDocuments.length === 1) {
            ...
            throw new Error(
                localize.DataScience.documentMismatch().format(file)
            );
        }
            ...
            if (pythonConfig) {
                ...
                for (const k of keys) {
                    ...
                    if (typeof currentValue === "string") {
                        const inspectResult = pythonConfig.inspect<string>(
                            `dataScience.${k}`
                        );
                        if (
                            inspectResult &&
                            inspectResult.defaultValue !== currentValue
                        ) {
