...
export class DataViewerProvider
    implements IDataViewerProvider, IAsyncDisposable {
    ...
    public async create(variable: string): Promise<IDataViewer> {
        ...
        if (index >= 0) {
            const dataExplorer = this.serviceContainer.get<IDataViewer>(
                IDataViewer
            );
        ...
        throw new Error(
            localize.DataScience.dataExplorerInvalidVariableFormat().format(
                variable
            )
        );
    }

    public async getPandasVersion(): Promise<
        { major: number; minor: number; build: number } | undefined
    > {
        ...
        try {
            ...
            const versionMatch = /^\s*(\d+)\.(\d+)\.(\d+)\s*$/.exec(
                result.stdout
            );
