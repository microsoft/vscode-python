...
export class CodeCssGenerator implements ICodeCssGenerator {
    ...
    public generateThemeCss(isDark: boolean, theme: string): Promise<string> {
        return this.applyThemeData(
            isDark,
            theme,
            "",
            this.generateCss.bind(this)
        );
    }

    public generateMonacoTheme(
        isDark: boolean,
        theme: string
    ): Promise<JSONObject> {
        return this.applyThemeData(
            ...
            theme,
            {},
            this.generateMonacoThemeObject.bind(this)
        );
    }
    ...
    private async applyThemeData<T>(
        ...
        try {
            ...
            const ignoreTheme = this.configService.getSettings().datascience
                .ignoreVscodeTheme
                ? true
                : false;
    ...
    private getScopes(entry: any): JSONArray {
        if (entry && entry.scope) {
            return Array.isArray(entry.scope)
                ? (entry.scope as JSONArray)
                : entry.scope.toString().split(",");
        }
        return [];
    }
    ...
    private getScopeStyle = (
        ...
        return {
            color: this.getDefaultColor(defaultStyle, scope),
            fontStyle: "normal"
        };
    };
    ...
    private generateCss(args: IApplyThemeArgs): string {
        ...
        const commentStyle = this.getScopeStyle(
            args.tokenColors,
            "comment",
            "comment",
            args.defaultStyle
        );
        const numericStyle = this.getScopeStyle(
            args.tokenColors,
            "constant.numeric",
            "constant",
            args.defaultStyle
        );
        const stringStyle = this.getScopeStyle(
            args.tokenColors,
            "string",
            "string",
            args.defaultStyle
        );
        const variableStyle = this.getScopeStyle(
            args.tokenColors,
            "variable",
            "variable",
            args.defaultStyle
...
`;
    ...
    private generateMonacoThemeObject(
        args: IApplyThemeArgs
        ...
        if (args.tokenColors && args.tokenColors.length) {
            ...
            result.rules = result.rules.sort(
                (
                    a: monacoEditor.editor.ITokenThemeRule,
                    b: monacoEditor.editor.ITokenThemeRule
                ) => {
        ...
        if (args.baseColors) {
            ...
            keys.forEach(k => {
                const color =
                    args.baseColors && args.baseColors[k]
                        ? args.baseColors[k]
                        : "#000000";
        ...
        try {
            ...
            if (themeRoot) {
                ...
                if (found !== null) {
                    ...
                    this.logger.logInformation(
                        `Reading colors from ${themeFile}`
                    );
                    return await this.readTokenColors(themeFile);
                }
            } else {
                this.logger.logWarning(
                    `Color theme ${theme} not found. Using default colors.`
                );
            }
        } catch (err) {
    ...
    private findBaseColors = async (
        ...
        try {
            ...
            if (themeRoot) {
                this.logger.logInformation(
                    `Loading base colors from ${themeRoot} ...`
                );
                ...
                if (found !== null) {
                    ...
                    this.logger.logInformation(
                        `Reading base colors from ${themeFile}`
                    );
