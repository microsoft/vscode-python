...
export class HistoryMessageListener implements IWebPanelMessageListener {
    ...
    constructor(
        ...
        this.postOffice = new PostOffice(
            LiveShare.WebPanelMessageService,
            liveShare,
            (api, _command, role, args) =>
        ...
        this.historyMessages.forEach(m => {
            this.postOffice
                .registerCallback(m, a => callback(m, a))
                .ignoreErrors();
        });
    }
    ...
    private getHistoryMessages(): string[] {
        return Object.keys(HistoryMessages).map(k =>
            (HistoryMessages as any)[k].toString()
        );
    }

    private translateHostArgs(
                            ...
                            if (file !== Identifiers.EmptyFileName) {
                                const uri =
                                    role === vsls.Role.Host
                                        ...
                                        : vscode.Uri.parse(`vsls:${file}`);

                                // Translate this into the other side.
                                trueArg[k] =
                                    role === vsls.Role.Host
                                        ? api.convertLocalUriToShared(uri)
                                              .fsPath
                                        : api.convertSharedUriToLocal(uri)
                                              .fsPath;
                            }
                        }
                    }
