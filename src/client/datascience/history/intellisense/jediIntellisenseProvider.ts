...
@injectable()
export class JediIntellisenseProvider extends BaseIntellisenseProvider
    implements IHistoryListener {
    private active: boolean = false;
    private pythonHoverProvider: PythonHoverProvider | undefined;
    private pythonCompletionItemProvider:
    ...
    constructor(
        ...
        this.jediFactory = new JediFactory(
        ...
        if (this.active) {
            this.pythonHoverProvider = new PythonHoverProvider(
                this.jediFactory
            );
            this.pythonCompletionItemProvider = new PythonCompletionItemProvider(
                this.jediFactory,
                this.serviceManager
            );
            this.pythonSignatureHelpProvider = new PythonSignatureProvider(
                this.jediFactory
            );
        }
    }
    ...
    protected async provideCompletionItems(
        ...
        if (this.pythonCompletionItemProvider && document) {
            ...
            const result = await this.pythonCompletionItemProvider.provideCompletionItems(
                document,
                docPos,
                token
            );
    ...
    protected async provideHover(
        ...
        if (this.pythonHoverProvider && document) {
            ...
            const result = await this.pythonHoverProvider.provideHover(
                document,
                docPos,
                token
            );
    ...
    protected async provideSignatureHelp(
        ...
        if (this.pythonSignatureHelpProvider && document) {
            ...
            const result = await this.pythonSignatureHelpProvider.provideSignatureHelp(
                document,
                docPos,
                token
            );
    ...
    protected handleChanges(
        _originalFile: string | undefined,
        _document: IntellisenseDocument,
