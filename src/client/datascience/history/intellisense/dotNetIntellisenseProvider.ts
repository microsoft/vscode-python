...
@injectable()
export class DotNetIntellisenseProvider extends BaseIntellisenseProvider
    implements IHistoryListener {
    private languageClientPromise:
        | Deferred<vscodeLanguageClient.LanguageClient>
        | undefined;
    ...
    constructor(
        ...
        this.configService
            ...
            .onDidChange(
                () =>
                    (this.active = !this.configService.getSettings()
                        .jediEnabled)
            );
    }
    ...
    protected async provideCompletionItems(
        ...
        if (languageClient && document) {
            const docPos = document.convertToDocumentPosition(
                cellId,
                position.lineNumber,
                position.column
            );
            const result = await languageClient.sendRequest(
                ...
                languageClient.code2ProtocolConverter.asCompletionParams(
                    document,
                    docPos,
                    context
                ),
    ...
    protected async provideHover(
        ...
        if (languageClient && document) {
            ...
            const result = await languageClient.sendRequest(
                ...
                languageClient.code2ProtocolConverter.asTextDocumentPositionParams(
                    document,
                    docPos
                ),
                token
    ...
    protected async handleChanges(
        ...
        if (this.active && document) {
            ...
            const languageClient = await this.getLanguageClient(
                originalFile === Identifiers.EmptyFileName ||
            ...
            if (!this.sentOpenDocument) {
                ...
                return languageClient.sendNotification(
                ...
                return languageClient.sendNotification(
                    ...
                    { textDocument: docItemId, contentChanges: changes }
                );
            }
        }
    }
    ...
    private getLanguageClient(
        file?: Uri
    ): Promise<vscodeLanguageClient.LanguageClient> {
        if (!this.languageClientPromise) {
            this.languageClientPromise = createDeferred<
                vscodeLanguageClient.LanguageClient
            >();
            this.startup(file)
                .then(() => {
                    this.languageClientPromise!.resolve(
