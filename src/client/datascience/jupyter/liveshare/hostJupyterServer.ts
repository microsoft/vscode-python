...
export class HostJupyterServer
    ...
    public async connect(
        ...
        if (
            ...
            const portMatch = RegExpValues.ExtractPortRegex.exec(
    ...
    public async onAttach(api: vsls.LiveShare | null): Promise<void> {
        ...
        if (api && !this.disposed) {
            ...
            if (service) {
                ...
                service.onRequest(
                    LiveShareCommands.syncRequest,
                    (_args: any[], _cancellation: CancellationToken) =>
                        this.onSync()
                );
                service.onRequest(
                    LiveShareCommands.getSysInfo,
                    (_args: any[], cancellation: CancellationToken) =>
                        this.onGetSysInfoRequest(cancellation)
                );
                service.onRequest(
                    LiveShareCommands.restart,
                    (args: any[], cancellation: CancellationToken) =>
                        this.onRestartRequest(
                            args.length > 0
                                ? (args[0] as number)
                                : LiveShare.InterruptDefaultTimeout,
                            cancellation
                        )
                );
                service.onRequest(
                    LiveShareCommands.interrupt,
                    (args: any[], cancellation: CancellationToken) =>
                ...
                service.onRequest(
                    LiveShareCommands.disposeServer,
                    (_args: any[], _cancellation: CancellationToken) =>
                        this.dispose()
                );
                ...
                service.onNotify(
                    LiveShareCommands.catchupRequest,
                    (args: object) => this.onCatchupRequest(args)
                );
                service.onNotify(
                    LiveShareCommands.executeObservable,
                    (args: object) => this.onExecuteObservableRequest(args)
                );
    ...
    private makeRequest(
        ...
        const observable = this.makeObservableRequest(
            code,
            file,
            line,
            id,
    ...
    private async attemptToForwardPort(
        api: vsls.LiveShare | null | undefined,
        port: number
    ): Promise<void> {
        if (
            port !== 0 &&
            api &&
            api.session &&
            api.session.role === vsls.Role.Host
        ) {
            this.portToForward = 0;
            this.sharedPort = await api.shareServer({
                port,
                displayName: localize.DataScience.liveShareHostFormat().format(
                    os.hostname()
                )
            });
        } else {
    ...
    private translateCellForGuest(cell: ICell): ICell {
        const copy = { ...cell };
        if (
            this.role === vsls.Role.Host &&
            this.finishedApi &&
            copy.file !== Identifiers.EmptyFileName
        ) {
            copy.file = this.finishedApi.convertLocalUriToShared(
                vscode.Uri.file(copy.file)
            ).fsPath;
        }
        return copy;
    }
    ...
    private async onCatchupRequest(args: object): Promise<void> {
        if (args.hasOwnProperty("since")) {
            ...
            if (service) {
                ...
                this.catchupResponses.send(
                    service,
                    this.translateForGuest.bind(this)
                );
        ...
        if (args.hasOwnProperty("code")) {
            ...
            if (!this.requestLog.has(obj.id)) {
                try {
                    ...
                    const file =
                        this.finishedApi &&
                        obj.file !== Identifiers.EmptyFileName
    ...
    private translateForGuest = (r: IServerResponse): IServerResponse => {
        ...
        if (er && er.cells) {
            return {
                cells: er.cells.map(this.translateCellForGuest, this),
                ...er
            };
        }
        return r;
    ...
    private postObservableNext(
        ...
        this.postResult(
            ServerResponseType.ExecuteObservable,
            {
                code,
                pos,
                type: ServerResponseType.ExecuteObservable,
                cells,
                id,
                time: Date.now()
            },
    ...
    private postObservableComplete(
        code: string,
        pos: number,
        id: string,
        responseQueues: ResponseQueue[]
    ...
    private postException(exc: any, responseQueues: ResponseQueue[]) {
        this.postResult(
            ...
            {
                type: ServerResponseType.Exception,
                time: Date.now(),
