...
export class JupyterServerBase implements INotebookServer {
    ...
    public async connect(
        ...
        if (this.launchInfo && this.launchInfo.connectionInfo) {
            this.connectionInfoDisconnectHandler = this.launchInfo.connectionInfo.disconnected(
                c => {
        ...
        if (this.session) {
            ...
            const idleTimeout = this.configService.getSettings().datascience
    ...
    public waitForIdle(timeoutMs: number): Promise<void> {
        return this.session
            ? this.session.waitForIdle(timeoutMs)
            : Promise.resolve();
    }
    ...
    public async getSysInfo(): Promise<ICell> {
        ...
        const versionCells = await this.executeSilently(
            `import sys\r\nsys.version`
        );
        // tslint:disable-next-line:no-multiline-string
        const pathCells = await this.executeSilently(
            `import sys\r\nsys.executable`
        );
        // tslint:disable-next-line:no-multiline-string
        const notebookVersionCells = await this.executeSilently(
        ...
        const notebookVersion =
            notebookVersionCells.length > 0
                ? this.extractStreamOutput(notebookVersionCells[0]).trimQuotes()
                : "";
        const pythonPath =
            versionCells.length > 0
    ...
    public async interruptKernel(timeoutMs: number): Promise<InterruptResult> {
        if (this.session) {
            ...
            const firstPending =
                this.pendingCellSubscriptions.length > 0
                    ? this.pendingCellSubscriptions[0]
                    : undefined;

            // Create a promise that resolves when the first pending cell finishes
            const finished = firstPending
        ...
        await this.executeSilently(
            useDark
                ...
                : `matplotlib.rcParams.update(${
                      Identifiers.MatplotLibDefaultParams
                  })`
        );
    }
    ...
    private executeSilently(
        ...
        // Attempt to evaluate this cell in the jupyter notebook
        const observable = this.executeObservableImpl(
            code,
    ...
    private async initialNotebookSetup(
        cancelToken?: CancellationToken
    ): Promise<void> {
        ...
        try {
            ...
            if (
                ...
                await this.changeDirectoryIfPossible(
                    this.launchInfo.workingDir
                );
            }
            ...
            await this.executeSilently(
                `import matplotlib${os.EOL}%matplotlib inline${os.EOL}${
                    Identifiers.MatplotLibDefaultParams
    ...
    private handleCodeRequest = (
        ...
        if (subscriber.isValid(this.sessionStartTime)) {
            ...
            if (
                this.launchInfo &&
                this.launchInfo.connectionInfo &&
                this.launchInfo.connectionInfo.localProcExitCode
            ) {
                // Not running, just exit
                const exitCode = this.launchInfo.connectionInfo
                    .localProcExitCode;
                subscriber.error(
                    this.sessionStartTime,
                    new Error(
                ...
                if (this.launchInfo && this.launchInfo.connectionInfo) {
                    ...
                    exitHandlerDisposable = this.launchInfo.connectionInfo.disconnected(
                        c => {
                    ...
                    request.onIOPub = (msg: KernelMessage.IIOPubMessage) => {
                        try {
                            if (
                                ...
                                jupyterLab.KernelMessage.isStreamMsg(msg)
                            ) {
                                ...
                                jupyterLab.KernelMessage.isUpdateDisplayDataMsg(
                                    msg
                                )
                            ) {
                                ...
                                jupyterLab.KernelMessage.isErrorMsg(msg)
                            ) {
                                this.handleError(
                            ...
                            if (msg.content.execution_count) {
                                subscriber.cell.data.execution_count = msg
