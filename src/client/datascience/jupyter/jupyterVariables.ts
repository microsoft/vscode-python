...
export class JupyterVariables implements IJupyterVariables {
    ...
    public async getDataFrameInfo(
        ...
        return this.runScript<IJupyterVariable>(
            ...
            [
                {
                    key: "_VSCode_JupyterValuesColumn",
                    value: localize.DataScience.valuesColumn()
                }
            ]
        );
    }
    ...
    private async runScript<T>(
        ...
        const results = await activeServer.execute(
            scriptText,
        ...
        if (cells.length > 0 && cells[0].data) {
            ...
            if (codeCell.outputs.length > 0) {
                ...
                if (
                    ...
                    const error = localize.DataScience.jupyterGetVariablesExecutionError().format(
                        stripped
                    );
                    traceError(error);
