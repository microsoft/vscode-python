...
export class InteractiveWindowCommandListener implements IDataScienceCommandListener {
    ...
    private waitForStatus<T>(promise: () => Promise<T>, format: string, file?: string, canceled?: () => void, skipInteractiveWindow?: boolean) : Promise<T> {
        const message = file ? format.format(file) : format;
        return this.statusProvider.waitWithStatus(promise, message, undefined, canceled, skipInteractiveWindow);
    }
    ...
    private async importNotebook() : Promise<void> {
        ...
        if (uris && uris.length > 0) {
            ...
            await this.waitForStatus(async () => {
                ...
                const contents = await interactiveWindow.importNotebook(uris[0].fsPath);
    ...
    private async importNotebookOnFile(file: string) : Promise<void> {
        if (file && file.length > 0) {
            await this.waitForStatus(async () => {
                const interactiveWindow = await this.interactiveWindowProvider.getOrCreateActive();
                const contents = await interactiveWindow.importNotebook(file);
                await this.viewDocument(contents);
