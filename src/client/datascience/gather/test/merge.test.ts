...
describe('SlicedExecution', () => {
    function cell(executionEventId: string, text: string, executionCount?: number, id?: string): IGatherCell {
        if (executionCount === undefined) executionCount = 1;
        return new LogCell({ executionCount, text, executionEventId, id });
    }

    function cellSlice(cell: IGatherCell, slice: LocationSet): CellSlice {
    ...
    describe('merge', () => {
        it('unions slices with different cells', () => {
            let slice1 = slicedExecution(cellSlice(cell('1', 'a = 1', 1), new LocationSet(location(1, 0, 1, 5))));
            let slice2 = slicedExecution(cellSlice(cell('2', 'b = 2', 2), new LocationSet(location(1, 0, 1, 5))));
        ...
        it('will not include the same locations from the same cell twice', () => {
            let slice1 = slicedExecution(cellSlice(cell('1', 'a = 1'), new LocationSet(location(1, 0, 1, 5))));
            let slice2 = slicedExecution(cellSlice(cell('1', 'a = 1'), new LocationSet(location(1, 0, 1, 5))));
            let merged = slice1.merge(slice2);
            expect(merged.cellSlices.length).to.equal(1);
            expect(merged.cellSlices[0].slice.size).to.equal(1);
        });

        it('considers two cells sharing ID and execution count but differing in execution event ' + 'ID to be different', () => {
            let slice1 = slicedExecution(cellSlice(cell('1', 'a = 1', 1, 'id1'), new LocationSet(location(1, 0, 1, 5))));
            let slice2 = slicedExecution(cellSlice(cell('2', 'a = 1', 1, 'id1'), new LocationSet(location(1, 0, 1, 5))));
            let merged = slice1.merge(slice2);
        ...
        it('will include complementary ranges from two slices of the same cell', () => {
            let slice1 = slicedExecution(cellSlice(cell('1', 'a = 1'), new LocationSet(location(1, 0, 1, 5))));
            let slice2 = slicedExecution(cellSlice(cell('1', 'a = 1'), new LocationSet(location(1, 0, 1, 4))));
            let merged = slice1.merge(slice2);
            expect(merged.cellSlices.length).to.equal(1);
        ...
        it('reorders the cells in execution order', () => {
            let slice1 = slicedExecution(cellSlice(cell('2', 'a = 1', 2), new LocationSet(location(1, 0, 1, 5))));
            let slice2 = slicedExecution(cellSlice(cell('1', 'a = 1', 1), new LocationSet(location(1, 0, 1, 4))));
            let merged = slice1.merge(slice2);
