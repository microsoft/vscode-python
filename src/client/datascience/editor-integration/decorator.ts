...
export class Decorator implements IExtensionActivationService, IDisposable {
    ...
    constructor(
        ...
        disposables.push(
            this.documentManager.onDidChangeTextEditorSelection(
                this.changedSelection,
                this
            )
        );
        disposables.push(
            this.documentManager.onDidChangeTextDocument(
    ...
    private changedDocument(e: vscode.TextDocumentChangeEvent) {
        if (
            ...
            e.document === this.documentManager.activeTextEditor.document
        ) {
            this.triggerUpdate(this.documentManager.activeTextEditor);
        }
    }
    ...
    private update(editor: vscode.TextEditor | undefined) {
        if (
            ...
            if (settings.decorateCells && settings.enabled) {
                ...
                const cells = generateCellRanges(
                    editor.document,
                ...
                const currentRange = cells
                    .map(c => c.range)
                    .filter(r => r.contains(editor.selection.anchor));
                const rangeTop =
                ...
                const startRanges = cells.map(
                    c => new vscode.Range(c.range.start, c.range.start)
                );
