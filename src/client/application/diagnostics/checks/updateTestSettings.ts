...
export const InvalidTestSettingsDiagnosticscServiceId =
    "InvalidTestSettingsDiagnosticscServiceId";
...
export class InvalidTestSettingDiagnosticsService extends BaseDiagnosticsService {
    ...
    constructor(
        ...
        private readonly application: IApplicationEnvironment,
        @inject(IPersistentStateFactory) stateFactory: IPersistentStateFactory,
        @inject(IDiagnosticHandlerService)
        @named(DiagnosticCommandPromptHandlerServiceId)
        private readonly messageService: IDiagnosticHandlerService<
            MessageCommandPrompt
        >,
        ...
        this.stateStore = stateFactory.createGlobalPersistentState<string[]>(
        ...
        const options = [
            {
                ...
                command: {
                    ...
                    invoke: async (): Promise<void> => {
                        const filesToBeFixed = await this.getFilesToBeFixed();
                        await Promise.all(
                            filesToBeFixed.map(file =>
                                this.fixSettingInFile(file)
                            )
                        );
                    }
                }
    ...
    public getSettingsFiles() {
        if (!this.workspace.hasWorkspaceFolders) {
            return this.application.userSettingsFile
                ? [this.application.userSettingsFile]
                : [];
        }
        return this.workspace
            .workspaceFolders!.map(item =>
                path.join(item.uri.fsPath, ".vscode", "settings.json")
            )
            .concat(
                this.application.userSettingsFile
                    ? [this.application.userSettingsFile]
    ...
    public async fixSettingInFile(filePath: string) {
        ...
        await this.fs.writeFile(
            filePath,
            fileContents.replace(setting, '"python.testing')
        );
