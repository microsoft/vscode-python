...
export class PythonCompletionItemProvider implements vscode.CompletionItemProvider {
    ...
    private static parseDocumentation(item: proxy.IAutoCompleteItem, resource: Uri): vscode.CompletionItem {
    ...
    public resolveCompletionItem(item: vscode.CompletionItem): vscode.CompletionItem {
        const docs = extractDocumentation(item);
        item.documentation = docs[1].length === 0 ? item.description : sigAndDocs[1];
        return item;
    }
