...
import { ConfigurationTarget, OutputChannel, QuickPickItem, Uri, window, workspace } from 'vscode';
...
export class ProductInstaller implements IInstaller {
    ...
    public static translateProductToModule(product: Product, purpose: ModuleNamePurpose): string {
        switch (product) {
    ...
    public translateProductToModuleName(product: Product, purpose: ModuleNamePurpose): string {
        return ProductInstaller.translateProductToModule(product, purpose);
    }
...
abstract class BaseInstaller {
    ...
    public async install(product: Product, resource?: Uri): Promise<InstallerResponse> {
        ...
        const moduleName = ProductInstaller.translateProductToModule(product, ModuleNamePurpose.install);
    ...
    public async isInstalled(product: Product, resource?: Uri): Promise<boolean | undefined> {
        ...
        try {
            moduleName = ProductInstaller.translateProductToModule(product, ModuleNamePurpose.run);
            // tslint:disable-next-line:no-empty
        } catch { }
...
class TestFrameworkInstaller extends BaseInstaller {
    ...
    protected getExecutableNameFromSettings(product: Product, resource?: Uri): string {
        ...
        if (!settingsPropNames.pathName) {
            ...
            return ProductInstaller.translateProductToModule(product, ModuleNamePurpose.run);
        }
        const settings = PythonSettings.getInstance(resource);
