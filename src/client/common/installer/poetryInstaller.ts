...
export class PoetryInstaller extends ModuleInstaller
    ...
    public async isSupported(resource?: Uri): Promise<boolean> {
        ...
        if (
            !(await this.fs.fileExists(
                path.join(workspaceFolder.uri.fsPath, poetryFile)
            ))
        ) {
            return false;
        }
    ...
    protected async isPoetryAvailable(workfolder: Uri) {
        try {
            ...
            const execPath = this.configurationService.getSettings(workfolder)
                .poetryPath;
            const result = await processService.exec(execPath, ["list"], {
                cwd: workfolder.fsPath
            });
    ...
    protected async getExecutionInfo(
        ...
        const execPath = this.configurationService.getSettings(resource)
            .poetryPath;
        return {
