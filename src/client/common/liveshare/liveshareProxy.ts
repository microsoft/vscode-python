...
export class LiveShareProxy implements vsls.LiveShare {
    ...
    public join(
        link: Uri,
        options?: vsls.JoinOptions | undefined
    ): Promise<void> {
    ...
    public async shareService(
        name: string
    ): Promise<vsls.SharedService | null> {
    ...
    public getSharedService(
        name: string
    ): Promise<vsls.SharedServiceProxy | null> {
    ...
    public registerContactServiceProvider(
        ...
        return this.realApi.registerContactServiceProvider(
            name,
    ...
    private async onSessionChanged(ev: vsls.SessionChangeEvent): Promise<void> {
        ...
        if (this.currentRole !== newRole) {
            ...
            if (this.currentRole === vsls.Role.Host) {
                await this.realApi.unshareService(
                    LiveShare.GuestCheckerService
                );
            }
            ...
            if (this.currentRole === vsls.Role.Host) {
                this.guestChecker = await this.realApi.shareService(
                    LiveShare.GuestCheckerService
                );
                if (this.guestChecker) {
                    this.guestChecker.onNotify(
                        LiveShareCommands.guestCheck,
                        (_args: object) => this.onGuestResponse()
                    );
                }

                // If guest, we need to list for requests.
            } else if (this.currentRole === vsls.Role.Guest) {
                this.guestChecker = await this.realApi.getSharedService(
                    LiveShare.GuestCheckerService
                );
                if (this.guestChecker) {
                    this.guestChecker.onNotify(
                        LiveShareCommands.guestCheck,
                        (_args: object) => this.onHostRequest()
                    );
                }
            }
        }
    ...
    private peersAreOkay(): Promise<boolean> {
        ...
        if (
            !this.guestChecker ||
            this.currentRole !== vsls.Role.Host ||
        ...
        setTimeout(
            this.validatePendingGuests.bind(this),
            this.peerTimeout ? this.peerTimeout : 1000
        );
        return this.peerCheckPromise.promise;
    }
        ...
        this.realApi
            ...
            .then(() => {
                ...
                this.applicationShell.showErrorMessage(
                    localize.DataScience.liveShareInvalid()
                );
            })
            .ignoreErrors();
    }
