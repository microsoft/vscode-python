...
export class AnalysisExtensionActivator implements IExtensionActivator {
    ...
    public async activate(context: vscode.ExtensionContext): Promise<boolean> {
        ...
        const clientOptions = await this.getAnalysisOptions(configuration);
        if (!clientOptions) {
            return false;
        }
    ...
    private async getAnalysisOptions(configuration: IConfigurationService): Promise<languageClient.LanguageClientOptions | undefined> {
        ...
        if (interpreter) {
            ...
            const pythonPath = configuration.getSettings().pythonPath;
