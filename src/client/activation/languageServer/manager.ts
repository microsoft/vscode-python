...
export class LanguageServerManager implements ILanguageServerManager {
    ...
    constructor(
        ...
        @inject(ILanguageServerExtension)
        private readonly lsExtension: ILanguageServerExtension
    ) {}
    public dispose() {
    ...
    protected loadExtensionIfNecessary() {
        if (this.languageServer && this.lsExtension.loadExtensionArgs) {
            this.languageServer.loadExtension(
                this.lsExtension.loadExtensionArgs
            );
        }
    }
    @debounceSync(1000)
    ...
    protected async startLanguageServer(): Promise<void> {
        this.languageServer = this.serviceContainer.get<ILanguageServer>(
            ILanguageServer
        );
