...
export class LanguageServerAnalysisOptions
    ...
    public async initialize(resource: Resource) {
        ...
        disposable = this.envVarsProvider.onDidEnvironmentVariablesChange(
            this.onEnvVarChange,
            this
        );
        this.disposables.push(disposable);
    ...
    public async getAnalysisOptions(): Promise<LanguageClientOptions> {
        ...
        const interpreterInfo = await this.interpreterService.getActiveInterpreter(
            this.resource
        );
        if (!interpreterInfo) {
        ...
        if (version) {
            // tslint:disable-next-line:no-string-literal
            properties["Version"] = `${version.major}.${version.minor}.${
                version.patch
            }`;
        } else {
            traceError(
                "Unable to determine Python version. Analysis may be limited."
            );
        }
        ...
        if (this.envPythonPath !== "") {
            const paths = this.envPythonPath
                .split(this.pathUtils.delimiter)
