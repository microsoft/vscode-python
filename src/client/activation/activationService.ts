...
export class LanguageServerExtensionActivationService
    ...
    public async activate(resource: Resource): Promise<void> {
        ...
        if (!jedi) {
            if (
                this.lsActivatedWorkspaces.has(
                    this.getWorkspacePathKey(resource)
                )
            ) {
                return;
            }
        ...
        let activatorName = jedi
            ? LanguageServerActivator.Jedi
            : LanguageServerActivator.DotNet;
        let activator = this.serviceContainer.get<ILanguageServerActivator>(
            ILanguageServerActivator,
            activatorName
        );
        ...
        try {
            ...
            if (!jedi) {
                this.lsActivatedWorkspaces.set(
                    this.getWorkspacePathKey(resource),
                    activator
                );
            }
            ...
            activator = this.serviceContainer.get<ILanguageServerActivator>(
                ILanguageServerActivator,
                activatorName
            );
            this.currentActivator = { jedi, activator };
    ...
    public async trackLangaugeServerSwitch(
        jediEnabled: boolean
    ): Promise<void> {
        ...
        if (state.value !== jediEnabled) {
            await state.updateValue(jediEnabled);
            const message = jediEnabled
                ? "Switch to Jedi from LS"
                : "Switch to LS from Jedi";
            sendTelemetryEvent(
    ...
    protected onWorkspaceFoldersChanged() {
        ...
        const workspaceKeys = this.workspaceService.workspaceFolders!.map(
            workspaceFolder => this.getWorkspacePathKey(workspaceFolder.uri)
        );
        const activatedWkspcKeys = Array.from(
            this.lsActivatedWorkspaces.keys()
        );
    ...
    private useJedi(): boolean {
        const configurationService = this.serviceContainer.get<
            IConfigurationService
        >(IConfigurationService);
        const enabled = configurationService.getSettings(this.resource)
            .jediEnabled;
