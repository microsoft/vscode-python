...
type PoetryInformation = { pythonPath: string; workspaceFolder: string; envName: string };
@injectable()
export class PoetryServiceHelper implements IPoetryServiceHelper {
    private initialized = false;
    private readonly state: IPersistentState<ReadonlyArray<PoetryInformation>>;
    constructor(
        @inject(IPersistentStateFactory) private readonly statefactory: IPersistentStateFactory,
        @inject(IFileSystem) private readonly fs: IFileSystem
    ) {
        this.state = this.statefactory.createGlobalPersistentState<ReadonlyArray<PoetryInformation>>(
            'PoetryInformation',
            []
        );
    }
    public async getPoetryInfo(pythonPath: string): Promise<{ workspaceFolder: Uri; envName: string } | undefined> {
    ...
    protected async initializeStateStore() {
        ...
        const filteredList = list.filter((item) => !!item) as PoetryInformation[];
