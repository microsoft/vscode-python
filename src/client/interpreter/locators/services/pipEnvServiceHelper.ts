...
export class PipEnvServiceHelper implements IPipEnvServiceHelper {
    ...
    constructor(
        ...
        this.state = this.statefactory.createGlobalPersistentState<
            ReadonlyArray<PipEnvInformation>
        >("PipEnvInformation", []);
    }
    public async getPipEnvInfo(
        pythonPath: string
    ): Promise<{ workspaceFolder: Uri; envName: string } | undefined> {
        await this.initializeStateStore();
        const info = this.state.value.find(item =>
            this.fs.arePathsSame(item.pythonPath, pythonPath)
        );
        return info
            ? {
                  workspaceFolder: Uri.file(info.workspaceFolder),
                  envName: info.envName
              }
            : undefined;
    }
    public async trackWorkspaceFolder(
        pythonPath: string,
        workspaceFolder: Uri
    ): Promise<void> {
        await this.initializeStateStore();
        const values = [...this.state.value].filter(
            item => !this.fs.arePathsSame(item.pythonPath, pythonPath)
        );
    ...
    protected async initializeStateStore() {
        ...
        const list = await Promise.all(
            this.state.value.map(
                async item =>
                    (await this.fs.fileExists(item.pythonPath))
                        ? item
                        : undefined
            )
        );
