...
export class InterpreterAutoSelectionService
    ...
    public async autoSelectInterpreter(resource: Resource): Promise<void> {
        ...
        if (!this.autoSelectedWorkspacePromises.has(key)) {
            ...
            await this.userDefinedInterpreter.autoSelectInterpreter(
                resource,
                this
            );
            this.didAutoSelectedInterpreterEmitter.fire();
            Promise.all(
                this.rules.map(item => item.autoSelectInterpreter(resource))
            ).ignoreErrors();
            deferred.resolve();
        }
    ...
    public async setWorkspaceInterpreter(
        ...
        if (
            !this.autoSelectedWorkspacePromises.has(
                this.getWorkspacePathKey(resource)
            ) ||
            !this.autoSelectedWorkspacePromises.get(
                this.getWorkspacePathKey(resource)
            )!.completed
        ) {
            return;
