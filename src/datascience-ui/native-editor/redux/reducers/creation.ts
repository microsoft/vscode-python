    ...
    export function deleteCell(arg: NativeEditorReducerArg<ICellAction>): IMainState {
        ...
        if (cells.length === 1 && cells[0].cell.id === arg.payload.data.cellId) {
            ...
            if (index >= 0) {
                ...
                if (selectionInfo.selectedCellId === arg.payload.data.cellId || selectionInfo.focusedCellId === arg.payload.data.cellId) {
                    if (nextOrPrev >= 0) {
                        newVMs[nextOrPrev] = { ...newVMs[nextOrPrev], selected: true, focused: selectionInfo.focusedCellId === arg.payload.data.cellId };
                    }
                }
