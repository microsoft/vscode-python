...
export class Cell extends React.Component<ICellProps> {
    ...
    private renderNormalCell() {
        ...
        const allowsPlainInput =
            getSettings().showCellInputCode ||
            this.props.cellVM.directInput ||
            this.props.cellVM.editable;
        const shouldRender =
            allowsPlainInput || (results && results.length > 0);
        const cellOuterClass = this.props.cellVM.editable
            ? "cell-outer-editable"
            : "cell-outer";
        const cellWrapperClass = this.props.cellVM.editable
            ? "cell-wrapper"
                ...
                >
                    <MenuBar baseTheme={this.props.baseTheme}>
                        <CellButton
                            ...
                            hidden={this.props.cellVM.editable}
                        >
                            <Image
                                baseTheme={this.props.baseTheme}
    ...
    private showInputs = (): boolean => {
        return (
            this.isCodeCell() &&
            (this.props.cellVM.inputBlockShow || this.props.cellVM.editable)
        );
    };
        ...
        if (mimetype) {
            ...
            try {
                ...
                if (output.data) {
                    ...
                    let data: nbformat.MultilineString | JSONObject =
                        mimeBundle[mimetype];
                    ...
                    if (forceLightTheme) {
                        ...
                        style.color = this.invertColor(
                            this.props.errorBackgroundColor
                        );
                    }

                    return (
                        <div id="stylewrapper" key={index} style={style}>
                            <Transform data={data} />
                        </div>
                    );
                }
    ...
    private renderOutput = (output: nbformat.IOutput, index: number) => {
        ...
        if (mimetype) {
            return this.renderWithTransform(
                mimetype,
                copy,
                index,
                addScrollbars,
