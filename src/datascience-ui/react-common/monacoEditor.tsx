...
> {
    ...
    public componentDidMount = () => {
        ...
        if (this.containerRef.current) {
            ...
            while (
                outerParent &&
                !outerParent.classList.contains(this.props.outermostParentClass)
            ) {
                outerParent = outerParent.parentElement;
            }
            this.outermostParent = outerParent;
            if (this.outermostParent) {
                this.outermostParent.addEventListener(
                    "mouseleave",
                    this.outermostParentLeave
                );
            }
                ...
                editor.onContextMenu(e => {
                    if (this.state.editor) {
                        ...
                        if (contextMenuElement) {
                            ...
                            contextMenuElement.style.top = `${Math.max(
                                0,
                                Math.floor(posY)
                            )}px`;
                            contextMenuElement.style.left = `${Math.max(
                                0,
                                Math.floor(posX)
                            )}px`;
                        }
            ...
            if (editorAny._standaloneKeybindingService) {
                editorAny._standaloneKeybindingService.addDynamicKeybinding(
                    "-actions.find"
                );
            }
    ...
    public componentWillUnmount = () => {
        ...
        if (this.outermostParent) {
            this.outermostParent.removeEventListener(
                "mouseleave",
                this.outermostParentLeave
            );
    ...
    public render() {
        return (
            <div
                className="monaco-editor-outer-container"
    ...
    private updateWidgetPosition(width?: number) {
        if (this.state.editor && this.widgetParent) {
            ...
            if (editorDomNode) {
                ...
                if (
                    rect &&
                    (rect.left !== this.lastOffsetLeft ||
                        rect.top !== this.lastOffsetTop)
                ) {
                    ...
                    this.widgetParent.setAttribute(
                        "style",
                        `position: absolute; left: ${rect.left}px; top: ${
                            rect.top
                        }px; width:${width ? width : rect.width}px`
                    );
                }
            }
        }
    ...
    private updateEditorSize = () => {
        if (
            ...
            const width =
                ...
                15; // Leave room for the scroll bar in regular cell table
    ...
    private onHoverLeave = () => {
        ...
        if (this.state.editor && this.widgetParent) {
            ...
            // tslint:disable-next-line: no-any
            const hover = this.state.editor.getContribution(
    ...
    private outermostParentLeave = () => {
        ...
        this.leaveTimer = window.setTimeout(
            this.outermostParentLeaveBounced,
            0
        );
    };
    ...
    private updateWidgetParent(
        ...
        if (editorNode) {
            try {
                const elements = editorNode.getElementsByClassName(
                    ...
                    if (contentWidgets) {
                        ...
                        if (document) {
                            ...
                            if (root) {
                                ...
                                this.widgetParent = document.createElement(
                                    "div",
                                    {}
                                );
                                this.widgetParent.setAttribute(
                                    "class",
                                    `${
                                        editorNode.className
                                    } monaco-editor-pretend-parent`
                                );
                                ...
                                // tslint:disable-next-line: no-any
                                const hover = editor.getContribution(
                                    "editor.contrib.hover"
                                ) as any;
                                if (
                                    hover._toUnhook &&
                                    hover._toUnhook.length === 8 &&
                                    hover.contentWidget
                                ) {
