...
When('I select the Python Interpreter containing the text {string}', async (text: string) => {
    await context.app.workbench.interpreters.selectInterpreter({ name: text });
});
...
Given('the package {string} is not installed', async (moduleName: string) => {
    await ensurePackageIsNotInstalled(moduleName);
});
When('I install the package {string}', async (moduleName: string) => {
    await ensurePackageIsInstalled(moduleName);
});
When('I uninstall the package {string}', async (moduleName: string) => {
    await ensurePackageIsInstalled(moduleName);
});
Given('the package {string} is installed', async (moduleName: string) => {
    await ensurePackageIsInstalled(moduleName);
});
