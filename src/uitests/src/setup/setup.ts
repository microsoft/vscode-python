...
import { noop, unzipFile, sleep } from '../helpers';
...
export class TestOptions implements ITestOptions {
    ...
    public async initilize() {
        this._workspacePathOrFolder = this._workspacePathOrFolder || path.join(this.tempPath, `workspace folder${this.workspaceCounter += 1}`);
        await Promise.all([
    ...
    public async updateForScenario(scenario: HookScenarioResult) {
        ...
        this._reportsPath = path.join(this.rootReportsPath, `${scenario.pickle.name}:${location}`.replace(/[^a-z0-9\-]/gi, '_'));
...
export function getTestOptions(channel: Channel, testDir: string, pythonPath: string = 'python', verboseLogging: boolean = false): ITestOptions {
    pythonPath = pythonPath || cp.execSync('python -c "import sys;print(sys.executable)"').toString().trim();
    // Where is VSC downloaded.
    // const vscodePath = getVSCodeDirectory(channel, testDir);
    ...
    [
        options.testPath, options.extensionsPath,
...
//     await downloadVSCode(options.channel, '');
...
export async function waitForPythonExtensionToActivate(timeout: number, app: IApplication) {
    ...
    await app.driver.$(selector, { retryTimeout: timeout });
