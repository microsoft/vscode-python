...
import { Channel, IApplication, IDebugger, IDocuments, IDriver, IInterpreters, ILocalization, INotifications, IPanels, IProblems, IQuickInput, IQuickOpen, ISettings, IStatusBar, ITerminal, ITestExplorer, ITestOptions } from '../types';
...
export class Application extends EventEmitter implements IApplication {
    ...
    public async captureScreenshot(name: string = 'screenshot'): Promise<Buffer> {
        ...
        name = `${previousCounter.toString().padStart(3, '0')}.${name}_${new Date().toISOString()}.png`;
        return this._driver.captureScreenshot(path.join(this.options.screenshotsPath, name));
    }
    /**
