...
export class Notifications implements INotifications {
    ...
    public hasMessages(): Promise<boolean> {
        return this.app.driver.$$(this.app.getCSSSelector(Selector.Notification))
            .then(eles => Array.isArray(eles) && eles.length > 0)
            .catch(() => false);
    }
    public getMessages(): Promise<string[]> {
        return this.app.driver.$$eval(this.app.getCSSSelector(Selector.IndividualNotification),
            elements => elements.map(ele => ele.textContent || '').filter(text => text.length > 0));
    }
    /**
    ...
    public async dismiss(messages: { content: string; buttonText?: string | undefined }[], timeout: number): Promise<void> {
        ...
        const _closeNotifications = async (): Promise<void> => {
            ...
            for (const i of [...new Array(count).keys()]) {
                ...
                const textContent = await this.app.driver.$eval(selector, element => element.textContent || '')
                ...
                const failed = await this.app.driver.click(closeSelector)
