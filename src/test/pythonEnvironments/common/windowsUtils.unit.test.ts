...
import { matchPythonExeFilename } from '../../../client/pythonEnvironments/common/windowsUtils';
...
suite('Windows Utils tests', () => {
    ...
    testParams.forEach((testParam) => {
        test(`Python executable check ${testParam.expected ? 'should match' : 'should not match'} this path: ${
            ...
            assert.deepEqual(matchPythonExeFilename(testParam.path), testParam.expected);
        });
    });
});
