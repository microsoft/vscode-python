...
async function ensureFSTreeIfWindows(tree: string): Promise<void> {
    if (IS_WINDOWS) {
        await ensureFSTree(tree.trimEnd(), __dirname);
    }
}

async function ensureFSTreeIfNotWindows(tree: string): Promise<void> {
    if (!IS_WINDOWS) {
        await ensureFSTree(tree.trimEnd(), __dirname);
    }
}
            ...
            await ensureFSTreeIfWindows(`
                ./.data/
                   ${rootName}/
                      ...
                      <python3.8.exe>
            `);
            await ensureFSTreeIfNotWindows(`
                ./.data/
                   ${rootName}/
                      sub1/
                         spam
