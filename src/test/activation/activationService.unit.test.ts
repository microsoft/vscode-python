...
suite('Language Server Activation - ActivationService', () => {
    [LanguageServerType.Jedi, LanguageServerType.Microsoft].forEach((languageServerType) => {
        suite(`Test activation - ${languageServerType === LanguageServerType.Jedi ? 'Jedi' : 'Microsoft'}`, () => {
            ...
            async function testActivation(
                ...
                lsSupported: boolean,
                activatorName: LanguageServerType
            ) {
                activator
                ...
                lsNotSupportedDiagnosticService
                    ...
                    .returns(() => Promise.resolve());
                serviceContainer
                    .setup((c) => c.get(TypeMoq.It.isValue(ILanguageServerActivator), TypeMoq.It.isAnyString()))
            ...
            async function testReloadMessage(settingName: string): Promise<void> {
                ...
                await testActivation(activationService, activator, true, languageServerType);
    ...
    suite('Test getLanguageServerType()', () => {
        ...
        test('If default value (Jedi) of language server is being used, and LSEnabled experiment is enabled, then return LanguageServerType.Jedi (no experiment)', async () => {
            const settings = { languageServer: undefined };
            experiments
        ...
        test('If default value of languageServer (Jedi) is being used, and LSEnabled experiment is disabled, then send telemetry if user is in Experiment LSControl and getLanguageServerType() should be Jedi)', async () => {
            const settings = { languageServer: undefined };
            experiments
                .setup((ex) => ex.inExperiment(LSEnabled))
                .returns(() => false)
        ...
        suite(
            'If default value of languageServer (Jedi) is not being used, then experiment is Microsoft -> Pylance, and Python settings value is returned',
            async () => {
                [
                    {
