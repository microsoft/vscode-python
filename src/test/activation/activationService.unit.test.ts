    ...
    [true, false].forEach(jediIsEnabled => {
        suite(`Jedi is ${jediIsEnabled ? "enabled" : "disabled"}`, () => {
            ...
            test("Track switch to LS", async () => {
                ...
                state.verify(
                    s => s.updateValue(TypeMoq.It.isValue(false)),
                    TypeMoq.Times.once()
                );
            });
            test("Track switch to Jedi", async () => {
                ...
                state.verify(
                    s => s.updateValue(TypeMoq.It.isValue(true)),
                    TypeMoq.Times.once()
                );
            });
            if (!jediIsEnabled) {
                ...
                async function testActivationOfResource(
                    ...
                    workspaceService
                        .setup(w =>
                            w.getWorkspaceFolderIdentifier(resource, "")
                        )
                ...
                test("Activator is disposed if activated workspace is removed", async () => {
                    ...
                    workspaceService
                        ...
                        .returns(
                            () => TypeMoq.Mock.ofType<IDisposable>().object
                        )
                        .verifiable(TypeMoq.Times.once());
                    const activationService = new LanguageServerExtensionActivationService(
                        serviceContainer.object,
                    ...
                    const activator1 = TypeMoq.Mock.ofType<
                        ILanguageServerActivator
                    >();
                    await testActivationOfResource(
                        activationService,
                        activator1,
                        folder1.uri
                    );
                    const activator2 = TypeMoq.Mock.ofType<
                        ILanguageServerActivator
                    >();
                    await testActivationOfResource(
                        activationService,
                    ...
                    const activator3 = TypeMoq.Mock.ofType<
                        ILanguageServerActivator
                    >();
                    await testActivationOfResource(
                        activationService,
                    ...
                    workspaceService
                        .setup(w =>
                            w.getWorkspaceFolderIdentifier(folder1.uri, "")
                        )
                        .returns(() => folder1.uri.fsPath)
                        .verifiable(TypeMoq.Times.atLeastOnce());
                    workspaceService
                        .setup(w =>
                            w.getWorkspaceFolderIdentifier(folder2.uri, "")
                        )
