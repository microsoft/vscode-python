...
suite("Virtual Environment Prompt", () => {
    ...
    test("User is not notified if python path is specified in settings.json", async () => {
        ...
        when(helper.getBestInterpreter(anything())).thenReturn(
            interpreter2 as any
        );
        when(workspaceService.getConfiguration("python", resource)).thenReturn(
            workspaceConfig.object
        );
        workspaceConfig
            .setup(c => c.inspect<string>("pythonPath"))
    ...
    test("User is notified if interpreter exists and only python path to global interpreter is specified in settings", async () => {
        ...
        const settings = {
            ...
            globalValue: "path/to/globalInterpreter"
        };
        const prompts = [
        ...
        const notificationPromptEnabled = TypeMoq.Mock.ofType<
    ...
    const testsForHasUserDefinedPath = [
        {
            testName:
                "Returns false when workspace folder setting equals 'python'",
            settings: { workspaceFolderValue: "python" },
            expectedResult: false
        },
        {
            testName:
                "Returns true when interpreter is provided in workspace folder setting",
        ...
        {
            testName:
                "Returns true when interpreter is provided in workspace setting",
        ...
        {
            testName:
                "Returns false when interpreter is provided in global setting",
    ...
    suite("Function hasUserDefinedPythonPath()", () => {
        testsForHasUserDefinedPath.forEach(testParams => {
            test(testParams.testName, async () => {
                ...
                >();
                when(
                    workspaceService.getConfiguration("python", resource)
                ...
                expect(
                    environmentPrompt.hasUserDefinedPythonPath(resource)
                ).to.equal(testParams.expectedResult);
