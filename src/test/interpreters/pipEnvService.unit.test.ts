            ...
            resource => {
                ...
                setup(() => {
                    ...
                    serviceContainer
                        .setup(c =>
                            c.get(
                                TypeMoq.It.isValue(IPipEnvServiceHelper),
                                TypeMoq.It.isAny()
                            )
                        )
                    ...
                    when(
                        pipEnvServiceHelper.trackWorkspaceFolder(
                            anything(),
                            anything()
                        )
                    ).thenResolve();
                ...
                test(`Should display warning message if there is a \'PipFile\' but \'pipenv --venv\' fails with stderr ${testSuffix}`, async () => {
                    ...
                    processService
                        ...
                        .returns(() =>
                            Promise.resolve({
                                stderr: "",
                                stdout: "pipenv, version 2018.11.26"
                            })
                        );
                    processService
                        .setup(p =>
                            p.exec(
                                TypeMoq.It.isValue("pipenv"),
                                TypeMoq.It.isValue(["--venv"]),
                                TypeMoq.It.isAny()
                            )
                        )
                        .returns(() =>
                            Promise.resolve({ stderr: "Aborted!", stdout: "" })
                        );
                    fileSystem
                        .setup(fs =>
                            fs.fileExists(
