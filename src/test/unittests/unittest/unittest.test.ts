...
suite('Unit Tests - unittest - discovery against actual python process', () => {
    ...
    test('Ensure correct test count for running a single test', async () => {
        ...
        const testRunResult: Tests = await testManager.runTest(CommandSource.ui, testsToRun);
        assert.equal(testRunResult.summary.errors, 2, 'This test was written assuming there was 2 tests run that would fail.');
        assert.equal(testRunResult.summary.passed, 2, 'This test was written assuming there was 2 tests run that would succeed.');
    });
});
