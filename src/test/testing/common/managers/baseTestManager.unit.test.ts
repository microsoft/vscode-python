    ...
    [
        ...
        suite(item.name, () => {
            ...
            setup(() => {
                ...
                when(
                    serviceContainer.get<OutputChannel>(
                        IOutputChannel,
                        TEST_OUTPUT_CHANNEL
                    )
                ).thenReturn(instance(outputChannel));
                when(
                    serviceContainer.get<ITestCollectionStorageService>(
                        ITestCollectionStorageService
                    )
                ).thenReturn(instance(storageService));
                when(
                    serviceContainer.get<ITestResultsService>(
                        ITestResultsService
                    )
                ).thenReturn(instance(resultsService));
                when(
                    serviceContainer.get<IWorkspaceService>(IWorkspaceService)
                ).thenReturn(instance(workspaceService));
                when(
                    serviceContainer.get<ITestDiagnosticService>(
                        ITestDiagnosticService
                    )
                ).thenReturn(instance(diagnosticService));
                when(
                    serviceContainer.get<ITestsStatusUpdaterService>(
                        ITestsStatusUpdaterService
                    )
                ).thenReturn(instance(statusUpdater));
                when(
                    serviceContainer.get<ICommandManager>(ICommandManager)
                ).thenReturn(instance(commandManager));
                ...
                ).thenReturn(instance(argsService));
                when(
                    serviceContainer.get<ITestsHelper>(ITestsHelper)
                ).thenReturn(instance(testsHelper));
                when(
                    serviceContainer.get<ITestManagerRunner>(
                        ITestManagerRunner,
                        anything()
                    )
                ).thenReturn(instance(runner));
                when(
                    serviceContainer.get<ITestMessageService>(
                        ITestMessageService,
                        anything()
                    )
            ...
            test("Discovering tests should display test manager", async () => {
                ...
                try {
                    await testManager.discoverTests(
                        CommandSource.auto,
                        true,
                        true,
                        true
