...
suite('Unit Tests - Test Helpers', () => {
    ...
    test('addPathToPythonpath adds path if it does not exist in the python path', async () => {
        ...
        const actualPath = addPathToPythonpath(newPaths, launchPythonPath);
        const expectedPath = path.join("random", "existing", "pythonpath") + path.delimiter + path.join("path", "to", "new");
    ...
    test('addPathToPythonpath splits pythonpath then rejoins it', async () => {
        const launchPythonPath = path.join("path", "to", "new") + path.delimiter + path.join("path", "to", "old") + path.delimiter + path.join("path", "to", "random");
        const actualPath = addPathToPythonpath(newPaths, launchPythonPath);
        const expectedPath = path.join("path", "to", "new") + path.delimiter + path.join("path", "to", "old") + path.delimiter + path.join("path", "to", "random");
        assert.equal(actualPath, expectedPath);
    });
});
