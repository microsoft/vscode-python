...
suite("Unit Tests - Context Service", () => {
    ...
    test("Status change without tests does not update hasFailedTests", async () => {
        ...
        await handler.bind(contextService)({
            status: TestStatus.Discovering,
    ...
    test("Status change with a summary updates hasFailedTests to false ", async () => {
        ...
        const fn = (cb: StatusChangeHandler) => (handler = cb);
        when(mgr.onDidStatusChange).thenReturn(fn as any);
        when(storage.getTests(anything())).thenReturn({
            summary: { failures: 0 }
        } as any);
        contextService.register();
        ...
        verify(
            cmdManager.executeCommand("setContext", "hasFailedTests", false)
        ).once();
    });
    test("Status change with a summary and failures updates hasFailedTests to false", async () => {
        ...
        verify(
            cmdManager.executeCommand("setContext", "hasFailedTests", true)
        ).once();
    });
    test("Status change with status of running", async () => {
        ...
        verify(
            cmdManager.executeCommand("setContext", "runningTests", true)
        ).once();
        verify(
            cmdManager.executeCommand("setContext", "discoveringTests", false)
        ).once();
        verify(
            cmdManager.executeCommand("setContext", "busyTests", true)
        ).once();
    });
    test("Status change with status of discovering", async () => {
        ...
        verify(
            cmdManager.executeCommand("setContext", "runningTests", false)
        ).once();
        verify(
            cmdManager.executeCommand("setContext", "discoveringTests", true)
        ).once();
        verify(
            cmdManager.executeCommand("setContext", "busyTests", true)
        ).once();
    ...
    test("Status change with status of others", async () => {
        ...
        verify(
            cmdManager.executeCommand("setContext", "busyTests", false)
        ).once();
