...
} from "../../../../client/testing/common/services/types";
import {
...
suite("Unit Tests - Common Discovery", () => {
    ...
    setup(() => {
        ...
        discovery = new TestsDiscoveryService(
    ...
    test("Use parser to parse results", async () => {
        ...
        when(
            parser.parse(options.workspaceFolder, deepEqual(discoveredTests))
        ).thenResolve(parsedResult as any);
    ...
    test("Invoke Python Code to discover tests", async () => {
        ...
        when(
            executionFactory.createActivatedEnvironment(
                deepEqual(creationOptions)
            )
        ).thenResolve(execService.object);
        const executionResult = { stdout: discoveredTests };
        execService
            .setup(e =>
                e.exec(
                    typemoq.It.isValue([pythonFile, ...options.args]),
                    typemoq.It.isValue(spawnOptions)
                )
            )
