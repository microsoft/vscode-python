...
suite("Unit Tests - Debug Launcher", () => {
    ...
    function getNewResolver(configService: IConfigurationService) {
        const validator = TypeMoq.Mock.ofType<
        ...
        return new LaunchConfigurationResolver(
            ...
            TypeMoq.Mock.ofType<IDocumentManager>(
                undefined,
                TypeMoq.MockBehavior.Strict
            ).object,
            TypeMoq.Mock.ofType<IConfigurationProviderUtils>(
                undefined,
                TypeMoq.MockBehavior.Strict
            ).object,
            validator.object,
    ...
    test("Ensure trailing commands in JSON are handled", async () => {
        ...
        filesystem
            .setup(fs => fs.fileExists(TypeMoq.It.isValue(filename)))
            .returns(() => Promise.resolve(true));
        filesystem
            .setup(fs => fs.readFile(TypeMoq.It.isValue(filename)))
            .returns(() => Promise.resolve(jsonc));

        const configs = await debugLauncher.readAllDebugConfigs(
            workspaceFolder
        );
