...
//                 { index: 0, name: 'test', uri: Uri.file(options.cwd) },
//             ];
//             getWorkspaceFoldersStub.returns(workspaceFolders);
//             getWorkspaceFolderStub.returns(workspaceFolders[0]);
//             pathExistsStub.resolves(false);
...
//         test('should use debugSessionName in config name when provided', async () => {
//             const options: LaunchOptions = {
//                 cwd: 'one/two/three',
//                 args: ['/one/two/three/testfile.py'],
//                 testProvider: 'pytest',
//                 runTestIdsPort: 'runTestIdsPort',
//                 pytestPort: 'pytestPort',
//                 debugSessionName: 'myproject (Python 3.11)',
//             };
...
//                 .setup((d) => d.onDidTerminateDebugSession(TypeMoq.It.isAny()))
//                 .returns((callback) => {
//                     // Immediately terminate with a matching session
//                     setTimeout(() => {
//                         if (capturedConfig) {
//                             callback({
//                                 id: 'test-session-id',
...
//             expect(capturedConfig).to.not.be.undefined;
//             expect(capturedConfig!.name).to.equal('Debug Tests: myproject (Python 3.11)');
//         });

//         test('should use pythonPath from options when provided', async () => {
//             const options: LaunchOptions = {
//                 cwd: 'one/two/three',
//                 args: ['/one/two/three/testfile.py'],
//                 testProvider: 'pytest',
//                 runTestIdsPort: 'runTestIdsPort',
//                 pytestPort: 'pytestPort',
//                 pythonPath: '/custom/python/interpreter',
//             };
...
//             expect(capturedConfig).to.not.be.undefined;
//             expect(capturedConfig!.python).to.equal('/custom/python/interpreter');
//         });
...
//             const marker = (capturedConfig as any).__vscodeTestSessionMarker;
