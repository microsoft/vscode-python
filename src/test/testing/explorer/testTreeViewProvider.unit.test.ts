...
class TestExplorerCaptureRefresh implements IDisposable {
    ...
    constructor(
        ...
        this.disposable = this.testViewProvider.onDidChangeTreeData(
            this.onRefreshOcured.bind(this)
        );
...
suite("Unit Tests Test Explorer TestTreeViewProvider", () => {
    suite("Misc", () => {
        ...
        test("Ensure that updates from the test manager propagate to the TestExplorer", async () => {
            ...
            when(workspaceService.getWorkspaceFolder(testResource)).thenReturn(
                workspaceFolder
            );
            ...
            const changeItem =
                testsData.testFolders[1].testFiles[0].functions[0];
            const testTreeProvider = createMockTestTreeProvider(
                instance(testStore),
                testsData,
                undefined,
            ...
            const refreshCap = new TestExplorerCaptureRefresh(
                testTreeProvider,
                disposables
            );

            testTreeProvider.refresh(testResource);
            const originalTreeItem = (await testTreeProvider.getTreeItem(
                changeItem
            )) as TestTreeItem;
            ...
            testTreeProvider.refresh(testResource);
            const changedTreeItem = (await testTreeProvider.getTreeItem(
                changeItem
            )) as TestTreeItem;
        ...
        test("A test file is added/removed/renamed", async () => {
            ...
            >();
            testStoreMoq
                .setup(a => a.getTests(typemoq.It.isAny()))
                .returns(() => testData);
            ...
            expect(unchangedItem).to.not.be.equal(
                undefined,
                "The file that will always be present, is not present."
            );
            ...
            let addedTreeItem = (await testTreeProvider.getTreeItem(
                fl2
            )) as TestTreeItem;
            ...
            addedTreeItem = (await testTreeProvider.getTreeItem(
                fl2
            )) as TestTreeItem;
            expect(addedTreeItem).to.not.be.equal(
                undefined,
        ...
        test("A test suite is added/removed/renamed", async () => {
            ...
            const sfn2 = getMockTestFunction(
                `test/test_fl.py::${origName}::test_fn`
            );
            const suite2 = getMockTestSuite(`test/test_fl.py::${origName}`, [
                sfn2
            ]);
            const fl1_update = getMockTestFile("test/test_fl.py", [
                suite,
                suite2
            ]);
            ...
            expect(unchangedItem).to.not.be.equal(
                undefined,
                "The suite that will always be present, is not present."
            );
        ...
        test("A test function is added/removed/renamed", async () => {
            ...
            const fl1_update = getMockTestFile(
                "test/test_fl.py",
                [],
                [fn, fn2]
            );
        ...
        test("A test status changes and is reflected in the tree view", async () => {
            ...
            const testFunction = getMockTestFunction(
                "test/test_file.py::test_fn"
            );
            testFunction.status = TestStatus.Pass;
            const testFile = getMockTestFile(
                "test/test_file.py",
                [],
                [testFunction]
            );
            ...
            testTreeProvider.refresh(testResource);
            const treeItem = (await testTreeProvider.getTreeItem(
                testFunction
            )) as TestTreeItem;
            ...
            testTreeProvider.refresh(testResource);
            let updatedTreeItem = (await testTreeProvider.getTreeItem(
                testFunction
            )) as TestTreeItem;
            ...
            testTreeProvider.refresh(testResource);
            updatedTreeItem = (await testTreeProvider.getTreeItem(
                testFunction
            )) as TestTreeItem;
        ...
        test("Get parent is working for each item type", async () => {
            ...
            const testFunction = getMockTestFunction(
                "test/test_file.py::test_suite::test_fn"
            );
            const testSuite = getMockTestSuite(
                "test/test_file.py::test_suite",
                [testFunction]
            );
            const outerTestFunction = getMockTestFunction(
                "test/test_file.py::test_outer_fn"
            );
            const testFile = getMockTestFile(
                "test/test_file.py",
                [testSuite],
        ...
        test("Get children is working for each item type", async () => {
            ...
            expect(children.length).to.be.equal(
                0,
                "A function should never have children."
            );

            children = await testTreeProvider.getChildren(testSuite);
            expect(children.length).to.be.equal(
                1,
                "Suite a single function should only return one child."
            );
        ...
        test("Tree items for subtests are correct", async () => {
            ...
            const testExplorer = createMockTestTreeProvider(
                undefined,
                testData
            );
            const items = [
        ...
        test("Children for subtests are correct", async () => {
            ...
            test("Get children will discover only once", async () => {
                ...
                const testStore = mock(TestCollectionStorageService);
                const testWorkspaceFolder = new TestWorkspaceFolder({
                    uri: Uri.file(__filename),
                    name: "",
                    index: 0
                });
                when(
                    testStore.getTests(testWorkspaceFolder.workspaceFolder.uri)
                ).thenReturn();
                when(testStore.onDidChange).thenReturn(noop as any);
                ...
                let tests = await testTreeProvider.getChildren(
                    testWorkspaceFolder
                );
        ...
        test("Expand tree item if it does not have any parent", async () => {
            ...
            expect(node.collapsibleState).to.equal(
        ...
        test("Expand tree item if the parent is the Workspace Folder in a multiroot scenario", async () => {
            ...
            const parentFolder = new TestWorkspaceFolder({
                name: "",
                index: 0,
                uri: Uri.file(__filename)
            });
        ...
        test("Do not expand tree item if it does not have any parent", async () => {
            ...
            expect(node.collapsibleState).to.not.equal(
                TreeItemCollapsibleState.Expanded
            );
        });
    });
    suite("Root Nodes", () => {
        ...
        test("Root folders are returned as children", async () => {
            ...
            const child2FolderPath = path.join(
                "a",
                "b",
                "root1",
                "child1",
