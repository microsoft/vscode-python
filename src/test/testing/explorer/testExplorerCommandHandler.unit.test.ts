...
suite("Unit Tests - Test Explorer Command Handler", () => {
    ...
    setup(() => {
        ...
        commandHandler = new TestExplorerCommandHandler(
            instance(cmdManager),
            instance(testResourceMapper)
        );
    });
    ...
    async function testOpeningTestNode(
        ...
        ).last()[1] as any) as Function;
        await handler.bind(commandHandler)(data);

        verify(
            cmdManager.executeCommand(expectedCommand, resource, data, true)
        ).once();
    }
