    ...
    filterdTestScenarios.forEach(scenario => {
        suite(scenario.scenarioName, async () => {
            ...
            suiteSetup(async () => {
                ...
                // Setup the parser.
                const workspaceService = ioc.serviceContainer.get<
                    IWorkspaceService
                >(IWorkspaceService);
                ...
                const discoveryOutput = fs
                    ...
                    .replace(/\\/g, "/");
                const discoveredTest: DiscoveredTests[] = JSON.parse(
                    discoveryOutput
                );
                options.workspaceFolder = vscode.Uri.file(
                    discoveredTest[0].root
                );
