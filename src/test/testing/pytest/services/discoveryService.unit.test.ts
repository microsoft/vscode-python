...
suite("Unit Tests - PyTest - Discovery", () => {
    ...
    test("Ensure discovery is invoked when there are no test directories", async () => {
        ...
        discoveryService.discoverTestsInTestDirectory = () =>
            Promise.resolve(discoveredTests);
    ...
    test("Ensure discovery is invoked when there are multiple test directories", async () => {
        ...
        when(helper.mergeTests(deepEqual(["Result A", "Result B"]))).thenReturn(
            "mergedTests" as any
        );
    ...
    test("Build collection arguments", async () => {
        ...
        when(
            argsService.filterArguments(
                deepEqual(options.args),
                TestFilter.discovery
            )
        ).thenReturn(filteredArgs);
    ...
    test("Discover using common discovery", async () => {
        ...
        when(
            serviceContainer.get<ITestDiscoveryService>(
                ITestDiscoveryService,
                "common"
            )
        ).thenReturn(instance(commonDiscoveryService));
        when(
            commonDiscoveryService.discoverTests(deepEqual(discoveryOptions))
        ).thenResolve(discoveredTests);

        const tests = await discoveryService.discoverTestsInTestDirectory(
            options
        );
