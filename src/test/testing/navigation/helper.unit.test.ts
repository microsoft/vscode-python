...
suite("Unit Tests - Navigation Helper", () => {
    ...
    setup(() => {
        ...
        helper = new TestNavigatorHelper(
            instance(docManager),
    ...
    test("Ensure file is opened", async () => {
        ...
        when(docManager.openTextDocument(anything())).thenResolve(
            doc.object as any
        );
        when(docManager.showTextDocument(doc.object)).thenResolve(
            editor.object as any
        );
    ...
    test("No symbols if symbol provider is not registered", async () => {
        ...
        const predicate = (s: SymbolInformation) =>
            s.kind === SymbolKind.Function && s.name === "";
    ...
    test("No symbols if no symbols", async () => {
        const token = new CancellationTokenSource().token;
        when(
            symbolProvider.provideDocumentSymbols(doc.object, token)
        ).thenResolve([] as any);
    ...
    test("Returns matching symbol", async () => {
        ...
        when(
            symbolProvider.provideDocumentSymbols(doc.object, token)
        ).thenResolve(symbols as any);

        const predicate = (s: SymbolInformation) =>
            s.kind === SymbolKind.Class && s.name === "2";
        const symbol = await helper.findSymbol(doc.object, predicate, token);
