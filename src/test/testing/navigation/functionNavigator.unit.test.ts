...
suite("Unit Tests - Navigation Function", () => {
    ...
    setup(() => {
        ...
        navigator = new TestFunctionCodeNavigator(
            instance(helper),
            instance(docManager),
            instance(storage)
        );
    ...
    test("Ensure file is opened", async () => {
        ...
        when(storage.findFlattendTestFunction(filePath, anything())).thenReturn(
            flattenedFn as any
        );
    ...
    async function navigateToFunction(focusCode: boolean) {
        ...
        const flattenedFn = {
            parentTestFile: { fullPath: filePath.fsPath },
            testFunction: { name: "function_name" }
        };
        when(storage.findFlattendTestFunction(filePath, anything())).thenReturn(
            flattenedFn as any
        );
    ...
    test("Ensure file is opened and range not revealed", async () => {
        ...
        const search = (s: SymbolInformation) =>
            s.kind === SymbolKind.Function && s.name === "Hello";
