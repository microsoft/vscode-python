...
suite("Unit Tests - Navigation Command Handler", () => {
    ...
    setup(() => {
        ...
        symbolProvider = new TestFileSymbolProvider(
    ...
    test("Ensure no symbols are returned when there are errors in running the code", async () => {
        when(configService.getSettings(anything())).thenThrow(
            new Error("Kaboom")
        );
        doc.setup(d => d.isUntitled)
            .returns(() => false)
    ...
    test("Ensure no symbols are returned when there are no symbols to be returned", async () => {
        ...
        when(
            processService.exec(pythonPath, anything(), anything())
        ).thenResolve(proc);
        ...
        verify(
            processService.exec(
                pythonPath,
                deepEqual(args),
                deepEqual({ throwOnStdErr: true, token })
            )
        ).once();
        expect(symbols).to.be.lengthOf(0);
        doc.verifyAll();
