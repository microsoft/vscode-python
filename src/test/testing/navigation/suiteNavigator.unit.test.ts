...
suite("Unit Tests - Navigation Suite", () => {
    ...
    setup(() => {
        ...
        navigator = new TestSuiteCodeNavigator(
            instance(helper),
            instance(docManager),
            instance(storage)
        );
    ...
    test("Ensure file is opened", async () => {
        ...
        const flattenedSuite = {
            parentTestFile: { fullPath: filePath.fsPath },
            testSuite: {}
        };
        when(storage.findFlattendTestSuite(filePath, anything())).thenReturn(
            flattenedSuite as any
        );

        await navigator.navigateTo(filePath, {} as any);

        verify(helper.openFile(anything())).once();
        expect(capture(helper.openFile).first()[0]!.fsPath).to.equal(
            filePath.fsPath
        );
    });
    ...
    async function navigateUsingLineFromSuite(focusCode: boolean) {
        ...
        when(helper.findSymbol(doc.object, anything(), anything())).thenResolve(
            symbol
        );

        await navigator.navigateTo(
            filePath,
            { name: "suite_name" } as any,
            focusCode
        );
        ...
        if (focusCode) {
            verify(
                docManager.showTextDocument(
                    doc.object,
                    deepEqual({ preserveFocus: false, selection: range })
                )
            ).once();
        } else {
            editor.verify(
                e => e.revealRange(range, TextEditorRevealType.Default),
                typemoq.Times.once()
            );
        }
    }
    ...
    test("Ensure file is opened and range not revealed", async () => {
        ...
        const search = (s: SymbolInformation) =>
            s.kind === SymbolKind.Class && s.name === "Hello";
        ...
        editor.verify(
            e => e.revealRange(typemoq.It.isAny(), typemoq.It.isAny()),
            typemoq.Times.never()
        );
    });
});
