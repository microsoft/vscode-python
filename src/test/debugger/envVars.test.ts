...
import { getEnvironmentVariables } from '../../client/debugger/extension/configuration/resolvers/helper';
...
suite('Resolving Environment Variables when Debugging', () => {
    ...
    let envParser: IEnvironmentVariablesService;
    ...
    setup(async () => {
        ...
        envParser = ioc.serviceContainer.get<IEnvironmentVariablesService>(IEnvironmentVariablesService);
    ...
    async function testBasicProperties(console: ConsoleType, expectedNumberOfVariables: number) {
        ...
        const envVars = await getEnvironmentVariables(envParser, pathUtils, mockProcess, args);
