    ...
    testsForLaunchProvider.forEach(testParams => {
        suite(testParams.testName, async () => {
            test("Test debug launcher args", async () => {
                const args = new DebuggerLauncherScriptProvider(
                    testParams.path
                ).getLauncherArgs({ host: "something", port: 1234 });
                const expectedArgs = [
            ...
            test("Test non-debug launcher args", async () => {
                const args = new NoDebugLauncherScriptProvider(
                    testParams.path
                ).getLauncherArgs({ host: "something", port: 1234 });
                const expectedArgs = [
                    testParams.expectedPath,
            ...
            test("Test remote debug launcher args (and do not wait for debugger to attach)", async () => {
                const args = new RemoteDebuggerLauncherScriptProvider(
                    testParams.path
                ).getLauncherArgs({
                    host: "something",
                    port: 1234,
