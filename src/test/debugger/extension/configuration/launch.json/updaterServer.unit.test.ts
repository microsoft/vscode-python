...
suite("Debugging - launch.json Updater Service", () => {
    ...
    test("Cursor is not positioned in the configurations array", async () => {
        ...
        const cursorPosition = helper.getCursorPositionInConfigurationsArray(
            document.object,
            new Position(0, 0)
        );
    ...
    test("Text to be inserted must not be prefixed nor suffixed with commas", async () => {
        ...
        const textToInsert = helper.getTextForInsertion(
            config,
            "InsideEmptyArray"
        );
    ...
    test("When inserting the debug config into the json file format the document", async () => {
        ...
        when(
            commandManager.executeCommand("editor.action.formatDocument")
        ).thenResolve();

        await helper.insertDebugConfiguration(
            document.object,
            new Position(0, 0),
            config
        );
