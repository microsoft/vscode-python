...
import { DebugAdapterFactory } from '../../../../client/common/experimentGroups';
...
suite('Debugging - Adapter Factory Registration', () => {
    ...
    test('Register Adapter Factory if inside the DA experiment', async () => {
        when(experimentsManager.inExperiment(DebugAdapterFactory.experiment)).thenReturn(true);
    ...
    test('Don\'t register the Adapter Factory if not inside the DA experiment', async () => {
        when(experimentsManager.inExperiment(DebugAdapterFactory.experiment)).thenReturn(false);
