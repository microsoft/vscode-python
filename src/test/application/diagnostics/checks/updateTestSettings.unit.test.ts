...
import {
    InvalidTestSettingDiagnosticsService,
    InvalidTestSettingsDiagnostic
} from "../../../../client/application/diagnostics/checks/updateTestSettings";
...
suite("Application Diagnostics - Check Test Settings", () => {
    ...
    setup(() => {
        ...
        when(
            stateFactory.createGlobalPersistentState(
                "python.unitTest.Settings",
                anything()
            )
        ).thenReturn(instance(storage));

        diagnosticService = new InvalidTestSettingDiagnosticsService(
    ...
    test("Get settings files that contain the old unitTest setting", async () => {
        ...
        when(appEnv.userSettingsFile).thenReturn("user.json");
        when(fs.readFile("user.json")).thenResolve(
            '{"python.unitTest.cwd":"blah"}'
        );
        when(
            fs.readFile(path.join(folder1.fsPath, ".vscode", "settings.json"))
        ).thenResolve('{"python.testing.cwd":"blah"}');
        when(
            fs.readFile(path.join(folder2.fsPath, ".vscode", "settings.json"))
        ).thenResolve('{"python.unitTest.pyTestArgs":[]}');
    ...
    test("None of the settings file need to be fixed", async () => {
        ...
        when(
            fs.readFile(path.join(folder2.fsPath, ".vscode", "settings.json"))
        ).thenResolve('{"python.testing.pyTestArgs":[]}');
        when(storage.value).thenReturn([]);
    ...
    test("Updates to the settings file will replace unitTest with testing", async () => {
        ...
        verify(
            fs.writeFile("user.json", '{"python.testing.cwd":"blah"}')
        ).once();
    });
});
