...
suite("Nuget Azure Storage Repository", () => {
    ...
    setup(() => {
        ...
        nugetService = typeMoq.Mock.ofType<INugetService>(
            undefined,
            typeMoq.MockBehavior.Strict
        );
    ...
    class FakeBlobStore {
        ...
        public listBlobsSegmentedWithPrefix(
            ...
            const result: BlobService.ListBlobsResult = {
    ...
    for (const [uri, setting, expected] of tests) {
        test(`Get all packages ("${uri}" / ${setting})`, async () => {
            if (uri.startsWith("https://")) {
                ...
                workspace
                    .setup(w => w.getConfiguration("http", undefined))
                    .returns(() => cfg.object);
                cfg.setup(c => c.get("proxyStrictSSL", true)).returns(
                    () => setting
                );
            }
            ...
            blobstore.results.forEach(r => {
                nugetService
                    .setup(n => n.getVersionFromPackageFileName(r.name))
                    .returns(() => version);
            });
            let actualURI = "";
            ...
            expect(packages).to.deep.equal([
                {
                    package: "Azarath",
                    uri: "eggs/spam/Azarath",
                    version: version
                },
                {
                    package: "Metrion",
                    uri: "eggs/spam/Metrion",
                    version: version
                },
                {
                    package: "Zinthos",
                    uri: "eggs/spam/Zinthos",
                    version: version
                }
            ]);
