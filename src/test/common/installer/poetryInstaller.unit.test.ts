...
suite("Module Installer - Poetry", () => {
    ...
    test("Is not supported when the poetry is not available (with stderr)", async () => {
        ...
        ).thenResolve({ stderr: "Kaboom", stdout: "" });
    ...
    test("Is supported", async () => {
        ...
        when(fileSystem.fileExists(anything())).thenResolve(true);
        when(processServiceFactory.create(uri)).thenResolve(
            instance(processService)
        );
        when(
            processService.exec("poetry path", anything(), anything())
        ).thenResolve({ stderr: "", stdout: "" });
