...
suite("Linter Availability Provider tests", () => {
    ...
    async function testForLinterPromptResponse(
        ...
        const linterInfo = new class extends LinterInfo {
            public testIsEnabled: boolean =
                promptAction === "enable" ? true : false;
        ...
        factoryMock
            .setup(f =>
                f.createWorkspacePersistentState(TypeMoq.It.isAny(), true)
            )
            .returns(() => notificationPromptEnabled.object);
        notificationPromptEnabled
            .setup(n => n.value)
            .returns(() => promptEnabled);
        const selections: ["enable", "ignore", "disablePrompt"] = [
            "enable",
            "ignore",
            "disablePrompt"
        ];
        ...
        if (promptEnabled) {
            appShellMock
                ...
                .returns(() =>
                    Promise.resolve(
                        promptAction
                            ? optButtons[selections.indexOf(promptAction)]
                            : undefined
                    )
                )
    ...
    test('Linter is left unconfigured and prompt is disabled when "Do not show again" is selected', async () => {
        ...
        expect(result).to.equal(
            expectedResult,
            "Expected promptToConfigureAvailableLinter to return false."
        );
    });
            ...
            .returns(() =>
                Promise.resolve(
                    options.promptAction
                        ? optButtons[selections.indexOf(options.promptAction)]
                        : undefined
                )
            )
            .verifiable(TypeMoq.Times.once());
