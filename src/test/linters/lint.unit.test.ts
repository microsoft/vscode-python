...
class TestFixture extends BaseTestFixture {
    ...
    constructor(workspaceDir = ".", printLogs = false) {
        ...
        const pythonToolExecService = TypeMoq.Mock.ofType<
            IPythonToolExecutionService
        >(undefined, TypeMoq.MockBehavior.Strict);
        ...
        this.pythonExecService = TypeMoq.Mock.ofType<IPythonExecutionService>(
            undefined,
            TypeMoq.MockBehavior.Strict
        );
    ...
    public makeDocument(product: Product, filename: string): TextDocument {
        ...
        if (product === Product.pydocstyle) {
            const dummyLine = TypeMoq.Mock.ofType<TextLine>(
                undefined,
                TypeMoq.MockBehavior.Strict
            );
            dummyLine.setup(d => d.text).returns(() => "    ...");
            doc.setup(s => s.lineAt(TypeMoq.It.isAny())).returns(
                () => dummyLine.object
            );
        }
        return doc.object;
...
suite("Linting Scenarios", () => {
    ...
    test("No linting with PyLint (enabled) when disabled at top-level", async () => {
        ...
        assert.equal(
            messages.length,
            0,
            `Unexpected linter errors when linting is disabled, Output - ${
                fixture.output
            }`
        );
    });
    ...
    async function testEnablingDisablingOfLinter(
        ...
        if (enabled) {
            assert.notEqual(
                messages.length,
                0,
                `Expected linter errors when linter is enabled, Output - ${
                    fixture.output
                }`
            );
        } else {
            assert.equal(
                messages.length,
                0,
                `Unexpected linter errors when linter is disabled, Output - ${
                    fixture.output
                }`
            );
        }
    ...
    for (const product of LINTERID_BY_PRODUCT.keys()) {
        for (const enabled of [false, true]) {
            test(`${enabled ? "Enable" : "Disable"} ${getProductName(
                product
            )} and run linter`, async function() {
    ...
    for (const useMinimal of [true, false]) {
        for (const enabled of [true, false]) {
            test(`PyLint ${enabled ? "enabled" : "disabled"} with${
                ...
                await testEnablingDisablingOfLinter(
                    fixture,
                    Product.pylint,
                    enabled
                );
    ...
    async function testLinterMessages(fixture: TestFixture, product: Product) {
        ...
        if (messagesToBeReceived.length === 0) {
            assert.equal(
                messages.length,
                0,
                `No errors in linter, Output - ${fixture.output}`
            );
        } else {
    ...
    async function testLinterMessageCount(
        ...
        assert.equal(
            ...
            `Expected number of lint errors does not match lint error count, Output - ${
                fixture.output
            }`
        );
    }
...
suite("Linting Products", () => {
    ...
    test("All linting products are represented by linters", async () => {
        for (const product of PRODUCTS) {
            // tslint:disable-next-line:no-any
            if (
                prodService.getProductType(product as any) !==
                ProductType.Linter
            ) {
                continue;
            }
            ...
            assert.notEqual(
                found,
                undefined,
                `did find linter ${Product[product as any]}`
            );
        }
    });
    ...
    test("All linters match linting products", async () => {
        for (const product of LINTERID_BY_PRODUCT.keys()) {
            ...
            assert.notEqual(
                prodType,
                undefined,
                `${Product[product]} is not not properly registered`
            );
            assert.equal(
                prodType,
                ProductType.Linter,
                `${Product[product]} is not a linter product`
            );
        }
    });
    ...
    test("All linting product names match linter IDs", async () => {
        for (const [product, linterID] of LINTERID_BY_PRODUCT) {
            ...
            assert.equal(
                prodName,
                linterID,
                "product name does not match linter ID"
            );
        }
    });
});
