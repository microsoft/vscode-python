...
async function getInfoForConfig(product: Product) {
    ...
    assert.notEqual(
        dirname,
        undefined,
        `tests not set up for ${Product[product]}`
    );

    const filename = path.join(
        dirname!,
        product === Product.pylint ? "file2.py" : "file.py"
    );
...
class TestFixture extends BaseTestFixture {
    ...
    private static newPythonExecFactory(
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(IBufferDecoder), TypeMoq.It.isAny())
            )
            .returns(() => decoder);
...
suite("Linting Functional Tests", () => {
    ...
    for (const product of LINTERID_BY_PRODUCT.keys()) {
        ...
        test(`${getProductName(
            product
        )} with config in root`, async function() {
            if (
                [
                    Product.bandit,
                    Product.mypy,
            ...
            const {
                ...
                messagesToBeReceived,
                origRCFile
            } = await getInfoForConfig(product);
            let rcfile = "";
