...
suite("Theme colors", () => {
    ...
    const settings: PythonSettings = new PythonSettings(
        undefined,
    ...
    setup(() => {
        ...
        cssGenerator = new CodeCssGenerator(
        ...
        test(themeName, async () => {
            ...
            if (shouldExist) {
                ...
                assert.equal(
                    actuallyDark,
                    isDark,
                    `Theme ${themeName} darkness is not ${isDark}`
                );
                workspaceConfig.reset();
                workspaceConfig
                    .setup(ws =>
                        ws.get<string>(TypeMoq.It.isValue("colorTheme"))
                    )
                    .returns(() => {
                        return themeName;
                    });
                workspaceConfig
                    .setup(ws =>
                        ws.get<string>(TypeMoq.It.isValue("fontFamily"))
                    )
                    .returns(() => {
                        return "Arial";
                    });
                ...
                workspaceConfig
                    .setup(ws =>
                        ws.get(TypeMoq.It.isAnyString(), TypeMoq.It.isAny())
                    )
                    .returns((_s, d) => {
                        return d;
                    });
                const theme = await cssGenerator.generateMonacoTheme(
                    isDark,
                    themeName
                );
                ...
                assert.ok(
                    commentPunctuation >= 0,
                    "No punctuation.comment found"
                );
            } else {
    ...
    test("Missing json theme", async () => {
        ...
        mockThemeFinder
            .setup(m => m.isThemeDark(TypeMoq.It.isAnyString()))
        ...
        assert.equal(
            matches!.length,
            2,
            "Wrong number of matches for for string color"
        );
