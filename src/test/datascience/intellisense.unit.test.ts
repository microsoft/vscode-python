...
suite("DataScience Intellisense Unit Tests", () => {
    ...
    setup(() => {
        ...
        languageServer
            .setup(l => l.start(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
            .returns(() => Promise.resolve());
        analysisOptions
            .setup(a => a.getAnalysisOptions())
            .returns(() => Promise.resolve({}));
        languageServer
            .setup(l => l.languageClient)
    ...
    function sendMessage<M extends IHistoryMapping, T extends keyof M>(
        type: T,
        payload?: M[T]
    ): Promise<void> {
    ...
    function addCell(code: string, id: string): Promise<void> {
        return sendMessage(HistoryMessages.AddCell, {
            fullText: code,
            currentText: code,
            file: "foo.py",
            id
    ...
    function addCode(
        ...
        return sendMessage(HistoryMessages.EditCell, {
            changes: [change],
    ...
    test("Add a single cell", async () => {
        ...
        expect(languageClient.getDocumentContents()).to.be.eq(
            "import sys\n\n\n",
            "Document not set"
        );
    });
    ...
    test("Add a cell and remove it", async () => {
        ...
        expect(languageClient.getDocumentContents()).to.be.eq(
            "import sys\nsys",
            "Removing a cell broken"
        );
        await addCell("import sys", "2");
        expect(languageClient.getDocumentContents()).to.be.eq(
            "import sys\nimport sys\nsys",
            "Adding a cell broken"
        );
