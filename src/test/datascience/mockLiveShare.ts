...
class MockLiveService implements vsls.SharedService, vsls.SharedServiceProxy {
    ...
    private changeIsServiceAvailableEmitter: EventEmitter<
        boolean
    > = new EventEmitter<boolean>();
    private requestHandlers: Map<string, vsls.RequestHandler> = new Map<
        string,
        vsls.RequestHandler
    >();
    private notifyHandlers: Map<string, vsls.NotifyHandler> = new Map<
        string,
        vsls.NotifyHandler
    >();
    ...
    public request(
        ...
        // See if any handlers.
        const handler = this.sibling
            ? this.sibling.requestHandlers.get(name)
            : undefined;
        if (handler) {
            return handler(
                args,
                cancellation
                    ? cancellation
                    : this.defaultCancellationSource.token
            );
        }
    ...
    public notify(name: string, args: object): void {
        // See if any handlers.
        const handler = this.sibling
            ? this.sibling.notifyHandlers.get(name)
            : undefined;
        if (handler) {
            handler(args);
        }
...
function checkArg(value: any, name: string, type?: ArgumentType) {
    if (!value) {
        ...
        if (type === "array") {
            ...
            throw new Error(
                `Argument \'${name}\' must be a a non-array object.`
            );
        } else if (typeof value !== type) {
            throw new Error(
                `Argument \'${name}\' must be type \'' + type + '\'.`
            );
        }
    }
}
...
class MockLiveShare
    implements vsls.LiveShare, vsls.Session, vsls.Peer, IDisposable {
    private static others: MockLiveShare[] = [];
    private static services: Map<string, MockLiveService[]> = new Map<
        string,
        MockLiveService[]
    >();
    ...
    public dispose() {
        MockLiveShare.others = MockLiveShare.others.filter(
            o => o._id !== this._id
        );
    }
    ...
    public convertLocalUriToShared(localUri: Uri): Uri {
        ...
        const file = localUri.fsPath.includes("/")
            ? path.basename(localUri.fsPath)
            : localUri.fsPath;
...
export class MockLiveShareApi implements ILiveShareTestingApi {
    ...
    public forceRole(role: vsls.Role) {
        ...
        if (role !== this.currentRole) {
            ...
            this.internalApi.onDidChangeSession(
                this.onInternalSessionChanged,
                this
            );
