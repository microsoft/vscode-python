...
} from "../../client/datascience/shiftEnterBanner";
...
function loadBanner(
    ...
    const persistService: typemoq.IMock<
        IPersistentStateFactory
    > = typemoq.Mock.ofType<IPersistentStateFactory>();
    const enabledState: typemoq.IMock<
    ...
    dataScienceSettings
        .setup(d => d.sendSelectionToInteractiveWindow)
        .returns(() => false);
    pythonSettings
    ...
    jupyterExecution
        ...
        .verifiable(
            executionCalled ? typemoq.Times.once() : typemoq.Times.never()
        );
