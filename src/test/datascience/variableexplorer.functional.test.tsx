...
suite("DataScience History variable explorer tests", () => {
    ...
    suiteSetup(function() {
        ...
        if (!isRollingBuild) {
            ...
            console.log(
                "Skipping Variable Explorer tests. Requires python environment"
            );
...
[50000 rows x 1 columns]`,
                ...
                {
                    name: "mynpArray",
                    value: `array([0.00000000e+00, 2.00004000e+00, 4.00008000e+00, ...,
       9.99959999e+04, 9.99980000e+04, 1.00000000e+05])`,
                    supportsDataExplorer: true,
                    type: "ndarray",
                    size: 54,
                    shape: "",
...
function openVariableExplorer(
    wrapper: ReactWrapper<any, Readonly<{}>, React.Component>
) {
    const varExp: VariableExplorer = wrapper
        .find("VariableExplorer")
        .instance() as VariableExplorer;
...
function verifyRow(
    rowWrapper: ReactWrapper<any, Readonly<{}>, React.Component>,
    targetVariable: IJupyterVariable
) {
    const rowCells = rowWrapper.find("div.react-grid-Cell");

    expect(rowCells.length).to.be.equal(
        5,
        "Unexpected number of cells in variable explorer row"
    );
...
function verifyCell(
    ...
    expect(cellHTML.firstChild.rawAttributes["value"] as string).to.be.equal(
        value,
        `${targetName} has an unexpected value in variable explorer cell`
    );
}
