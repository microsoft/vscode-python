...
export function setUpDomEnvironment() {
    ...
    require.extensions[".js"] = function(mod: any, filename) {
        if (configurationRegex.test(filename)) {
            ...
            content +=
                "export function getCSSBasedConfiguration() { return CSSBasedConfiguration.INSTANCE; };\n";
...
export function createKeyboardEvent(
    ...
    const keyCode = keyMap.hasOwnProperty(upper)
        ? keyMap[upper].code
        : options.key!.charCodeAt(0);
    const shift = keyMap.hasOwnProperty(upper)
        ? keyMap[upper].shift || options.shiftKey
        : options.shiftKey;
    ...
    return new domWindow.KeyboardEvent(type, ({
        ...options,
        keyCode,
        shiftKey: shift
    } as any) as KeyboardEventInit);
}
