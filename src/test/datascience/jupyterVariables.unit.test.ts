...
suite("JupyterVariables", () => {
    ...
    function generateVariableOutput(
        outputData: string,
        outputType: string
    ): nbformat.IOutput {
        return {
    ...
    function generateCell(
        ...
        return {
            data: {
                ...
                metadata: {},
                outputs: hasOutput
                    ? [generateVariableOutput(outputData, outputType)]
                    : [],
                source: ""
            },
            id: "0",
    ...
    setup(() => {
        ...
        jupyterVariables = new JupyterVariables(
            fileSystem.object,
            execution.object,
    ...
    test("getVariables bad output type", async () => {
        ...
        fakeServer
            ...
            .returns(() =>
                Promise.resolve(
                    generateCells("bogus string", "bogus output type")
                )
            )
            .verifiable(typemoq.Times.once());
    ...
    test("getValue fake data", async () => {
        ...
        assert.deepEqual(resultVariable, {
            name: "big_complex",
            size: 60,
            type: "complex",
            value: "(1+1j)"
