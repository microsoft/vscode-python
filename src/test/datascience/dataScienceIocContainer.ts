...
export class DataScienceIocContainer extends UnitTestIocContainer {
    ...
    public registerDataScienceTypes() {
        ...
        this.serviceManager.addSingleton<IDataViewerProvider>(
            IDataViewerProvider,
            DataViewerProvider
        );
        ...
        this.serviceManager.addSingleton<ILiveShareApi>(
            ILiveShareApi,
            MockLiveShareApi
        );
        this.serviceManager.addSingleton<IExtensions>(
            IExtensions,
            MockExtensions
        );
        ...
        this.serviceManager.add<IJupyterCommandFactory>(
            IJupyterCommandFactory,
            JupyterCommandFactory
        );
        this.serviceManager.addSingleton<IThemeFinder>(
            IThemeFinder,
            ThemeFinder
        );
        ...
        this.serviceManager.addSingleton<IPipEnvServiceHelper>(
            IPipEnvServiceHelper,
            PipEnvServiceHelper
        );
        this.serviceManager.addSingleton<ILanguageServer>(
            ILanguageServer,
            MockLanguageServer
        );
        ...
        this.serviceManager.add<IHistoryListener>(
            IHistoryListener,
        ...
        workspaceService
            ...
            .returns(() => this.configChangeEvent.event);
        interpreterDisplay
        ...
        this.serviceManager.addSingletonInstance<IDataScience>(
            IDataScience,
            datascience.object
        );
        ...
        if (this.mockJupyter) {
            this.mockJupyter.addInterpreter(
                this.workingPython,
    ...
    public createWebView(
        ...
        if (role !== vsls.Role.None) {
            const liveShareTest = this.get<ILiveShareApi>(
                ILiveShareApi
            ...
            .returns(
                (
                    ...
                    if (this.missedMessages.length && this.webPanelListener) {
                        this.missedMessages.forEach(
                            m =>
                                this.webPanelListener
                                    ? this.webPanelListener.onMessage(
                                          m.type,
                                          m.payload
                                      )
                                    : noop()
    ...
    private postMessageToWebPanel(msg: any) {
        ...
        if (
            this.wrapperCreatedPromise &&
            !this.wrapperCreatedPromise.resolved
        ) {
            this.wrapperCreatedPromise.resolve();
        }
    }

    private mountReactControl(
        mount: () => ReactWrapper<any, Readonly<{}>, React.Component>
    ) {
