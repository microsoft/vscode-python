...
suite("DataScience notebook tests", () => {
    ...
    async function verifyCell(
        ...
        if (cellType === "code") {
            ...
            assert.ok(
                data || text,
                `${index}: No data object on the cell for ${code}`
            );
            if (data) {
                // For linter
                assert.ok(
        ...
        runTest("MimeTypes", async () => {
            ...
            if (server) {
                for (let i = 0; i < types.length; i += 1) {
                    const markdownRegex = types[i].markdownRegEx
                        ? types[i].markdownRegEx
                        : "";
    ...
    test("Not installed", async () => {
        ...
        class EmptyInterpreterService implements IInterpreterService {
            ...
            public getDisplayName(
                _interpreter: Partial<PythonInterpreter>
            ): Promise<string> {
                throw new Error("Method not implemented");
            }
    ...
    runTest("Export/Import", async () => {
        ...
        const testState = generateTestState(_id => {
            return;
        }, testFolderPath);
        ...
        if (notebook) {
            ...
            if (nbcells) {
                // tslint:disable-next-line:no-string-literal
                const firstCellText: string = (nbcells as any)[0][
                    "source"
                ] as string;
    ...
    runTest("Restart kernel", async () => {
        ...
        try {
            ...
            assert.ok(
                ...
                "Restarting did not timeout correctly"
            );
        }
    });
    ...
    async function testCancelableCall<T>(
        ...
        try {
            // tslint:disable-next-line:no-string-literal
            (tokenSource.token as any)["tag"] = messageFormat.format(
                timeout.toString()
            );
    ...
    runTest("Cancel execution", async () => {
        ...
        assert.ok(
            await testCancelableMethod(
                (t: CancellationToken) =>
                    jupyterExecution.connectToNotebookServer(undefined, t),
    ...
    async function interruptExecute(
        ...
        let error;
        const observable = server!.executeObservable(
            code,
            "foo.py",
            0,
            uuid(),
...
while keep_going:
  ...
  time.sleep(.1)`;
        ...
        addInterruptableMockData(
            fourSecondSleep,
            async (_cancelToken: CancellationToken) => {
...
echo 'hello'`,
            ...
            verifyValue: d =>
                assert.ok(
                    d.includes("hello"),
                    `Multiline cell magic incorrect - ${d}`
                )
        },
        {
            ...
            result: "world",
            verifyValue: d =>
                assert.ok(d.includes("world"), "Cell command incorrect")
        },
        {
            // Plotly
            markdownRegEx: undefined,
...
plt.show()`,
    ...
    runTest("Server cache working", async () => {
        ...
        assert.ok(
            s1 !== s4,
            "Different purpose should create different server"
        );
