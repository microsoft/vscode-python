...
suite("DataScience LiveShare tests", () => {
    ...
    function createContainer(role: vsls.Role): DataScienceIocContainer {
        ...
        result.serviceManager.rebindInstance<IApplicationShell>(
            IApplicationShell,
            appShell.object
        );
    ...
    function getOrCreateHistory(role: vsls.Role): Promise<IHistory> {
        ...
        const container =
            role === vsls.Role.Host ? hostContainer : guestContainer;
        return container!
            .get<IHistoryProvider>(IHistoryProvider)
            .getOrCreateActive();
    }
    ...
    function isSessionStarted(role: vsls.Role): boolean {
        const container =
            role === vsls.Role.Host ? hostContainer : guestContainer;
        const api = container!.get<ILiveShareApi>(
            ILiveShareApi
        ) as ILiveShareTestingApi;
    ...
    async function waitForResults(
        ...
        if (!guestStarted) {
            const hostRenderPromise = waitForUpdate(
                hostContainer.wrapper!,
                MainPanel,
                expectedRenderCount
            );
            ...
            const guestRenderPromise = waitForUpdate(
                guestContainer.wrapper!,
                MainPanel,
                expectedRenderCount
            );
            ...
            await Promise.all([
                hostRenderPromise,
                isSessionStarted(vsls.Role.Guest)
                    ? guestRenderPromise
                    : Promise.resolve()
            ]);
        }
            ...
            async (both: boolean) => {
                if (!both) {
                    ...
                    if (isSessionStarted(vsls.Role.Guest)) {
                        ...
                        return role === vsls.Role.Host
                            ? host.addCode(code, "foo.py", 2)
                            : guest.addCode(code, "foo.py", 2);
                    } else {
    ...
    test("Going through codewatcher", async () => {
        ...
        assert.ok(
            hostContainer.wrapper,
            "Host wrapper not created for some reason"
        );
        verifyHtmlOnCell(
            hostContainer.wrapper!,
            "<span>1</span>",
    ...
    test("Export from guest", async () => {
        ...
        guestContainer!.serviceManager.rebindInstance<IFileSystem>(
            IFileSystem,
            fileSystem.object
        );
        fileSystem
        ...
        const listener = guestContainer!.get<IDataScienceCommandListener>(
            IDataScienceCommandListener
        );
        const guestCommandManager = guestContainer!.get<ICommandManager>(
            ICommandManager
        );
        ...
        guestContainer!
            .get<IDocumentManager>(IDocumentManager)
            .showTextDocument(Uri.file("foo.py"));
    ...
    test("Guest does not have extension", async () => {
        ...
        verifyHtmlOnCell(wrapper, "<span>1</span>", CellPosition.Last);
        assert.equal(
            isSessionStarted(vsls.Role.Host),
            false,
            "Host should have exited session"
        );
        assert.equal(
            isSessionStarted(vsls.Role.Guest),
            false,
            "Guest should have exited session"
        );
        assert.ok(
            lastErrorMessage,
            "Error was not set during session shutdown"
        );
    });
});
