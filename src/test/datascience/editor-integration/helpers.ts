...
export function createDocument(
    ...
    const textLines = inputLines.map((line, index) => {
        ...
        textLine
            .setup(l => l.isEmptyOrWhitespace)
            .returns(() => line.trim().length === 0);
        return textLine;
    });
    document
        .setup(d => d.lineAt(TypeMoq.It.isAnyNumber()))
        .returns((index: number) => textLines[index].object);
