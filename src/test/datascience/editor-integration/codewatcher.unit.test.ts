...
suite("DataScience Code Watcher Unit Tests", () => {
    ...
    setup(() => {
        ...
        historyProvider
            ...
            .returns(() => Promise.resolve(activeHistory.object));
    ...
    function verifyCodeLensesAtPosition(
        ...
        if (codeLenses[startLensIndex + indexAdd].command) {
            expect(
                ...
                "Run Below code lens command incorrect"
            );
        }
        expect(codeLenses[startLensIndex + indexAdd].range).to.be.deep.equal(
            targetRange,
            "Run Below code lens range incorrect"
        );
    }
...
# fifth line`;
        pythonSettings.datascience.codeRegularExpression =
            "(#\\s*\\<foobar\\>|#\\s*\\<baz\\>)";
        pythonSettings.datascience.markdownRegularExpression =
            "(#\\s*\\<markdowncell\\>|#\\s*\\<mymarkdown\\>)";
