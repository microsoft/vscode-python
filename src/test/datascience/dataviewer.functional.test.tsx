...
suite("DataScience DataViewer tests", () => {
    ...
    suiteSetup(function() {
        ...
        if (!isRollingBuild) {
            ...
            console.log(
                "Skipping DataViewer tests. Requires python environment"
            );
    ...
    async function injectCode(code: string): Promise<void> {
        ...
        const server = await exec.connectToNotebookServer(
            await historyProvider.getNotebookOptions()
        );
        if (server) {
            const cells = await server.execute(
                code,
    ...
    function verifyRows(
        ...
        for (let i = 0; i < rows.length; i += 1) {
            ...
            const span = cells[i].querySelector(
                "div.cell-formatter span"
            ) as HTMLSpanElement;
            ...
            assert.equal(
                val,
                span.innerHTML,
                `Row ${i} not matching. ${span.innerHTML} !== ${val}`
            );
        }
    }
