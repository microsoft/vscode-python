...
suite("Formatting - Test Arguments", () => {
    ...
    setup(() => {
        ...
        when(appShell.setStatusBarMessage(anything(), anything())).thenReturn({
            dispose: noop
        });
        ...
        when(
            container.get<IOutputChannel>(
                IOutputChannel,
                STANDARD_OUTPUT_CHANNEL
            )
        ).thenReturn(instance(outputChannel));
        when(container.get<IApplicationShell>(IApplicationShell)).thenReturn(
            instance(appShell)
        );
        when(container.get<IFormatterHelper>(IFormatterHelper)).thenReturn(
            formatterHelper
        );
        when(container.get<IWorkspaceService>(IWorkspaceService)).thenReturn(
            instance(workspace)
        );
        when(
        ...
        when(
            container.get<IPythonToolExecutionService>(
                IPythonToolExecutionService
            )
        ).thenReturn(instance(pythonToolExecutionService));
        when(
            container.get<IDisposableRegistry>(IDisposableRegistry)
        ).thenReturn([]);
    });
    ...
    async function setupFormatter(
        ...
        when(
            pythonToolExecutionService.exec(anything(), anything(), anything())
        ).thenResolve({ stdout: "" });
    ...
    test("Ensure blackPath and args used to launch the formatter", async () => {
        ...
        const execInfo = await setupFormatter(
            formatter,
            formattingSettingsWithPath
        );
        ...
        assert.deepEqual(
            execInfo.args,
            formattingSettingsWithPath.blackArgs.concat([
                "--diff",
                "--quiet",
                docUri.fsPath
            ])
    ...
    test("Ensure black modulename and args used to launch the formatter", async () => {
        ...
        const execInfo = await setupFormatter(
            formatter,
            formattingSettingsWithModuleName
        );

        assert.equal(
            execInfo.execPath,
            formattingSettingsWithModuleName.blackPath
        );
        assert.equal(
            execInfo.moduleName,
            formattingSettingsWithModuleName.blackPath
    ...
    test("Ensure autopep8path and args used to launch the formatter", async () => {
        ...
        assert.equal(
            execInfo.execPath,
            formattingSettingsWithPath.autopep8Path
        );
        assert.equal(execInfo.moduleName, undefined);
        assert.deepEqual(
            execInfo.args,
            formattingSettingsWithPath.autopep8Args.concat([
                "--diff",
                docUri.fsPath
            ])
        );
    ...
    test("Ensure autpep8 modulename and args used to launch the formatter", async () => {
        ...
        assert.equal(
            execInfo.execPath,
            formattingSettingsWithModuleName.autopep8Path
        );
        assert.equal(
            execInfo.moduleName,
            formattingSettingsWithModuleName.autopep8Path
    ...
    test("Ensure yapfpath and args used to launch the formatter", async () => {
        ...
        assert.deepEqual(
            execInfo.args,
            formattingSettingsWithPath.yapfArgs.concat([
                "--diff",
                docUri.fsPath
            ])
        );
    ...
    test("Ensure yapf modulename and args used to launch the formatter", async () => {
        ...
        assert.equal(
            execInfo.execPath,
            formattingSettingsWithModuleName.yapfPath
        );
        assert.equal(
            execInfo.moduleName,
            formattingSettingsWithModuleName.yapfPath
