...
fn get_known_conda_install_locations(environment: &dyn known::Environment) -> Vec<PathBuf> {
    ...
    if let Some(home) = environment.get_user_home() {
        ...
        known_paths.push(PathBuf::from(home).join("miniforge3"));
    }
    known_paths
}
...
/**
 * Given a directory to the conda installation, return the conda manager along with the environments.
 */
pub fn get_conda_environments(environment: &dyn known::Environment) -> Option<LocatorResult> {
    ...
    for possible_conda_folder in get_known_conda_install_locations(environment) {
        if possible_conda_folder.exists() {
            if let Some(manager) = get_conda_manager(&possible_conda_folder) {
                ...
                if let Some(root_python_env) =
                    get_root_python_environment(&possible_conda_folder, &manager)
                {
                    environments.push(root_python_env);
                }
