...
function getModifiedFilesSync() {
    if (process.env.TRAVIS) {
        const cmd = process.env.TRAVIS_PULL_REQUEST === 'true' ?
            `git --no-pager diff --name-only FETCH_HEAD $(git merge-base FETCH_HEAD ${process.env.TRAVIS_BRANCH})` :
            `git diff --name-only master ${process.env.TRAVIS_BRANCH}`;
