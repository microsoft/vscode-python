...
gulp.task("compile", done => {
    ...
    tsProject
        ...
        .on(
            ...
            () =>
                failed
                    ? done(new Error("TypeScript compilation errors"))
                    : done()
        );
});
...
gulp.task("hygiene", done =>
    run(
        { mode: "compile", skipFormatCheck: true, skipIndentationCheck: true },
        done
    )
);
...
gulp.task("uploadDeveloperExtension", () =>
    uploadExtension("ms-python-insiders.vsix")
);
gulp.task("uploadReleaseExtension", () =>
    uploadExtension(
        `ms-python-${process.env.TRAVIS_BRANCH ||
            process.env.BUILD_SOURCEBRANCHNAME}.vsix`
    )
);
...
async function checkDatascienceDependencies() {
    ...
    json.children.forEach(c => {
        c.chunks[0].modules.forEach(m => {
            ...
            const nameWithoutNodeModules = name.substring(
                "./node_modules".length
            );
