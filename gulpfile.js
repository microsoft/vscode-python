...
gulp.task('installNewDebugpy', async () => {
    ...
    if (!successWithoutWheels) {
        console.info("Failed to install source only version of new DEBUGPY using 'python3', attempting to install using 'python'");
        await spawnAsync('python', args).catch(ex => console.error("Failed to install source only DEBUGPY 5.0 using 'python'", ex));
    }
});
